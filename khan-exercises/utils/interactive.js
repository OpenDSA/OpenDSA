define(function(require) {
    ...
    $.extend(KhanUtil.Graphie.prototype, {
        ...
        initAutoscaledGraph: function(range, options) {
            ...
            options.gridStep = [
                (options.range[0][1] - options.range[0][0]) / options.xdivisions, (options.range[1][1] - options.range[1][0]) / options.ydivisions
            ];
        ...
        addMovablePoint: function(options) {
            ...
            var normalColor = (movablePoint.constraints.fixed) ?
                KhanUtil.DYNAMIC : KhanUtil.INTERACTIVE;
        ...
        interactiveEllipse: function(options) {
            ...
            $(ellipse.perim[0]).on(
                "vmouseover vmouseout vmousemove",
                function(event) {
                    if (event.type === "vmouseover") {
            ...
            return function(options) {
                ...
                var redrawRotateHandle = function(handleCoord) {
                    ...
                    drawnRotateHandle = drawRotateHandle(
                        graph,
                        rotatePoint.coord,
                        options.radius, (isRotating || isHovering ?
    ...
    function Ruler(graphie, options) {
        ...
        var rightTop = kvector.add(
            leftBottom, [options.units * graphieUnitsPerUnit, graphieUnitsHeight]
        );
