...
called the B-:math:`+` tree.
When greater efficiency is required, a more complicated
variant known as the B=:math:`*` tree is used.
...
The B-:math:`+` tree is essentially a mechanism for managing a sorted
array-based list, where the list is broken into chunks.

The most significant difference between the B-:math:`+` tree and the
BST or the standard B-tree is that
the B-:math:`+` tree  stores records only at the leaf nodes.
...
with a pointer to a child B-:math:`+` tree node.
Leaf nodes store actual records, or else keys and pointers to actual
records in a separate disk file if the B-:math:`+` tree is being used
purely as an index.
Depending on the size of a record as compared to the size of a key,
a leaf node in a B-:math:`+` tree of order :math:`m` might have enough
room to store more or less than :math:`m` records.
The requirement is simply that the leaf nodes store enough records to
remain at least half full.
The leaf nodes of a B-:math:`+` tree are normally
...
Here is a Java-like pseudocode representation for the B-:math:`+` tree
node interface.
...
Figure :num:`Figure #BTexamp` shows the B-:math:`+` tree as it is
traditionally drawn.
...
B-:math:`+` tree implementations typically store an additional dummy
record in the leftmost leaf node whose key value is less than any
legal key value.

B-:math:`+` trees are exceptionally good for range queries.
...
Figure :num:`Figure #BPexamp` illustrates the B-:math:`+` tree.
...
.. odsafig:: Images/BPexamp.png
   ...
   :capalign: justify
   :figwidth: 90%
   :alt: Example of a B-:math:`+` tree.

   Example of a B-:math:`+` tree of order four.
...
Search in a B-:math:`+` tree is nearly identical to search in a
regular B-tree, except that the search must always continue to the
proper leaf node.
...
To find a record with key value 33 in the B-:math:`+` tree of
...
Here is a pseudocode sketch of the B-:math:`+` tree search algorithm.

.. codeinclude:: Indexing/BPfind.pde

B-:math:`+` tree insertion is similar to B-tree insertion.
First, the leaf :math:`L` that should contain the record is found.
If :math:`L` is not full, then the new record is added, and no
other B-:math:`+` tree nodes are affected.
...
the root and causing the B-:math:`+` tree to gain a new level.
B-:math:`+` tree insertion keeps all leaf nodes at equal depth.
Figure :num:`Figure #BPins` illustrates the insertion process through several
examples.
...
.. odsafig:: Images/BPins.png
   ...
   :capalign: justify
   :figwidth: 90%
   :alt: Examples of B-:math:`+` tree insertion.


   Examples of B-:math:`+` tree insertion.
   (a) A B-:math:`+` tree containing five records.
   ...
   (c) The B-:math:`+` tree of (b) after further insertions.
...
Here is a a Java-like pseudocode sketch of the B-:math:`+` tree
insert algorithm.

.. codeinclude:: Indexing/BPinsert.pde

To delete record :math:`R` from the B-:math:`+` tree,
...
.. odsafig:: Images/BPsimDel.png
   ...
   :capalign: justify
   :figwidth: 90%
   :alt: Simple deletion from a B-:math:`+` tree.

   Simple deletion from a B-:math:`+` tree.
...
.. odsafig:: Images/BPborrow.png
   ...
   :capalign: justify
   :figwidth: 90%
   :alt: Deletion from a B-:math:`+` tree via borrowing from a sibling.

   Deletion from the B-:math:`+` tree of Figure 
...
.. odsafig:: Images/BPmerge.png
   ...
   :capalign: justify
   :figwidth: 90%
   :alt: Deletion from a B-:math:`+` tree via collapsing siblings

   Deleting the record with key value 33 from the B-:math:`+` tree of
   Figure :num:`Figure #BPexamp` via collapsing siblings.
...
Here is a Java-like pseudocode for the B-:math:`+` tree delete algorithm.

.. codeinclude:: Indexing/BPremove.pde

The B-:math:`+` tree requires that all nodes be at least half full
(except for the root).
...
One method for doing so is to use the B-:math:`+` tree variant known
as the B-:math:`*` tree.
The B-:math:`*` tree is identical to the B-:math:`+` tree, except for
the rules used to split and merge nodes.
Instead of splitting a node in half when it overflows, the B-:math:`*`
tree
...
Here is a visualization for the B-:math:`+` tree.
...
records from B-trees, B-:math:`+` trees, and B-:math:`*` trees is
:math:`\Theta(\log n)`
...
As an illustration, consider a B-:math:`+` tree of order 100 and leaf
nodes that contain up to 100 records.
A B-:math:`+` tree with height one (that is, just a single leaf node)
can have at most 100 records.
A B-:math:`+` tree with height two (a root internal node whose
children are leaves) must have at least 100 records
(2 leaves with 50 records each).
It has at most 10,000 records (100 leaves with 100 records each).
A B-:math:`+` tree with height three must have at least 5000 records
...
children each).
A B-:math:`+` tree with height four must have at least 250,000 records
and at most 100 million records.
Thus, it would require an *extremely* large database to generate
a B-:math:`+` tree of more than height four.

The B-:math:`+` tree split and insert rules guarantee that every node
(except perhaps the root) is at least half full.
...
nodes, in a B-:math:`+` tree of order 100 probably only about
