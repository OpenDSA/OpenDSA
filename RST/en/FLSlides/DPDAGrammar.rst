...
.. slide:: Deterministic vs. Non-deterministic PDAs and Grammars
   ...
   | Now we know that:
   |    All CFL can be generated by a CFL, and implemented
        by a NPDA.
   |    Not all CFL can be defined using a DPDA.
...
.. slide:: PDAs and Parsing
   ...
   | Think of PDAs as a parsing device.
   |    No backtracking :math:`=` can make a decision at every step on
        what to do next.
   |    This is the same as knowing grammar production comes next.
...
.. slide:: LL Example 2 (1)
   ...
   | This is **not** an LL(k) grammar for an :math:`k`.
   |    << Why not?? >>
