...
string, there is also a blank character.
...
To do the computation, we have to have some conventions about starting and
ending the process.
...
Is every Turing-acceptable language Turing decidable?
This is the Halting Problem.

Of course, if the Turing-acceptable language would halt,
we write :math:`\fbox{Y}`.
But if the Turing-acceptable language would hang,
...
* :math:`M_2` passes control to :math:`M_1` with I/O head at the end of input.
...
.. inlineav:: Turing3CON dgm
   ...
   left, and then transition to the machine :math:`M`.
...
* Multiple tapes (each with its own head)
  ...
  each cell the corresponding two symbols, and a two-binary marker to
