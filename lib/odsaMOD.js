...
(function($) {
  ...
  /**
   * Array of exercises names that student become proficient in. This array is loaded from the server by loadModule function.
   */
  ...
  function loadModule(modName) {
    ...
    if (odsaUtils.scoringServerEnabled()) {
      if (odsaUtils.isFullModule()) {
        url = "/odsa_exercise_progresses?inst_chapter_module_id=" + odsaUtils.getChapterModuleID() +
          "&inst_book_id=" + odsaUtils.getBookID();
      }
      else {
        url = "/odsa_exercise_progresses/" + odsaUtils.getBookID() + "/" + odsaUtils.getSectionID();
      }
  ...
  $(document).ready(function() {
    ...
    if (ODSA.SETTINGS.MODULE_NAME === 'Glossary') {
      ...
      if (localStorage.getItem('glossaryTarget') !== null) {
        ...
        odsaUtils.logUserAction('Out-glossary-link', 'User loaded the glossary link from module page');
        var container = $('html'),
      ...
      odsaUtils.logUserAction('direct-glossary-link', 'User loaded the glossary link from the direct glossary page');    
    }
    ...
    if (ODSA.SETTINGS.MODULE_NAME !== 'Glossary') {
      ...
      $('body').click(function(event) {
        if ($(event.target).is('.xref, .std, .std-term')) {
          ...
          odsaUtils.logUserAction('glossary-term-clicked', 'User loaded a glossary term link'); 
        }
      });
    }
  });
