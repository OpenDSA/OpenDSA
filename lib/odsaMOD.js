...
(function ($) {
  ...
  function updateProfDisplay(name, proficient, error) {
    ...
    if (exercises[name]) { // name refers to an exercise
      ...
      if (proficient) {
        window.dispatchEvent(new CustomEvent('odsa-exercise-complete', { detail: { exercise: name } }));
      }
  ...
  function loadModule(modName) {
    if (!odsaUtils.hasBook() && !odsaUtils.isStandaloneModule()) return;
  ...
  function processEventData(data) {
    if (!data.av) {
      return; // if av is undefined, note: the av property in data is the key to identify the exercises in the exercises dict
    }
    var flush = false;
    ...
    if (odsaUtils.ssEvents.indexOf(data.type) > -1) {
      // When the SPLICE protocol is active (the page runs inside a learning
      // system iframe and splice-iframe.js has created window.SPLICE), the
      // slideshow position and completion are reported through SPLICE by the
      // listener in odsaUtils.js. Skip the legacy score submission and the
      // legacy per-click event logging to avoid double reporting; keep the
      // local proficiency display update.
      var spliceActive = !!(window.SPLICE && window.SPLICE.sendEvent);

      data.desc.currentStep = data.currentStep;
      data.desc.totalSteps = data.totalSteps;

      if (spliceActive) {
        // Position reporting is handled by SPLICE.sendEvent; mark the event
        // as logged so the legacy logEvent() call below skips it.
        data.logged = true;
      }
      ...
      if (data.currentStep === data.totalSteps && ((settings.REQ_FULL_SS && exercises[data.av].highestStep === data.totalSteps) || !settings.REQ_FULL_SS)) {
        ...
        if (!exercises[data.av].hasOwnProperty('complete')) {
          if (spliceActive) {
            // The completion score is reported via SPLICE.reportScoreAndState
            // (odsaUtils.js); only update the local proficiency display here.
  ...
  function exerciseLoaded(exerName) {
    ...
    //Set the width to 100% if the iFrame is not an embedContainer inorder to center the exercises
    if ($iframe.closest('.embedContainer').length === 0) {
  ...
  $(document).ready(function () {
    ...
    // Initialize exercise with default settings if it's not present in the exercises dict
    // Note: the exercises will only behave like this (missing objects) because they are served outside of the DOM/module, which is most likely our exercises in the Live catalogue
    function initializeExerciseIfMissing(avName) {
      if (!exercises[avName]) {
        console.warn('Exercise running outside of OpenDSA Books for:', avName, '- initializing with defaults.');
        exercises[avName] = {
          name: avName,           // set avName as the default name
          points: "0.0",          // Default points if not specified
          required: false,        // Default to not required
          threshold: 0.0,         // Default threshold
          type: "standalone",     // Mark as standalone (live catalogue)
          oembed: undefined,      // Default oembed
          id: null,               // Default id
          uiid: +new Date()       // Assign unique identifier
        };
      }
    }
    ...
    eventer(messageEvent, function (e) {
      ...
      if (data) {
        if (data.subject === 'lti.frameResize') {
            // Handle frame resize messages separately
        ...
        // Handle SPLICE messages separately
        if (data.subject === 'SPLICE.getState') {
            return;
        }
        if (data.subject === 'SPLICE.reportScoreAndState') {
          // Extract data from SPLICE reportScoreAndState
          const state = data.state;
          if (state) {
              // KA exercises save their own attempts directly via the
              // HTTP path (Khan.saveAttemptToServer). A KA attempt
              // arriving through SPLICE is informational only — do not
              // re-submit it to the server, or the attempt is recorded
              // (and graded) twice.
              const spliceState = state[Object.keys(state)[0]] || {};
              if (spliceState.type === 'ka-exercise-attempt') {
                  console.log('SPLICE KA attempt (informational; already saved directly):', spliceState);
                  ltiIframeResize();
                  return;
              }
              const exerciseName = Object.keys(state)[0];
              const exerciseData = state[exerciseName];

              // Assign fields with fallbacks for missing fields, most likely only happens for external exercises e.g Codecheck
              data.av = exerciseData.av || exerciseName || 'unknown';
              data.type = exerciseData.type || data.type || 'jsav-exercise-grade';
              data.currentStep = exerciseData.currentStep !== undefined ? exerciseData.currentStep : 0;
              data.totalTime = exerciseData.totalTime || 0;
              data.tstamp = exerciseData.tstamp || new Date().toISOString();
              data.uiid = exerciseData.uiid || Math.floor(Date.now() / 1000);

              if (data.type === 'jsav-exercise-grade' || data.type === 'jsav-exercise-grade-change') {
                  if (exerciseData.score) {
                      data.score = exerciseData.score;
                      data.desc = data.desc || {};
                      data.desc.score = exerciseData.score.correct / exerciseData.score.total;
                      data.desc.complete = (exerciseData.score.correct + exerciseData.score.undo + exerciseData.score.fix) / exerciseData.score.total;
                  } else if (data.score !== undefined) {
                      data.score = { correct: data.score, total: 1 };
                      data.desc = data.desc || {};
                      data.desc.score = data.score.correct / data.score.total;
                      data.desc.complete = data.score.correct;
                  } else {
                      console.warn("Missing score data in SPLICE.reportScoreAndState:", data);
                      data.score = { correct: 0, total: 1 };
                      data.desc = data.desc || {};
                      data.desc.score = 0;
                      data.desc.complete = 0;
                  }
              }
              data.desc = data.desc || {};
              processEventData(data);
              ...
              console.error("State data is missing in SPLICE.reportScoreAndState:", data);
          }
          return;
        }
        if (data.subject === 'SPLICE.sendEvent') {
            return;
        }
        ...
        if (data.type === "resize-iframe") {
          ...
          initializeExerciseIfMissing(data.av);
          ...
          if (exercises[data.av]) {
            ...
            processEventData(data);  //uncomment this to return from splice message to jsav messages
          } else {
              console.warn('undefined object:', data.av);
          }
        } else {
            console.warn('Received message without av:', data);
        }
      }
    }, false);
  ...
  function initExercises(elem) {
    ...
    if ($elem.attr('data-oembed') === 'true') {
      ...
      $(iFrameSelector).on("load", function (event) {
