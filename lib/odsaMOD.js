...
(function ($) {
  ...
  const sendScoreData = (data) => {
    ...
    const scoreData = {
      ...
      score:       data.score,
      steps_fixed: data.state.currentStep,
      tstamp:      (new Date(data.state.tstamp)).getTime(),
      total_time:  window.TimeMe.getTimeOnPageInMilliseconds(location),
      threshold:   window.PARAMS.threshold,
      uiid:        +new Date(),
      type:        data.state.isKA ? "AE" : "PE"
    };

    if (window.ODSA.UTILS.hasBook()) {
      scoreData['inst_book_id'] 
        = window.ODSA.TP.instBookId || data.state.inst_book_id;
      scoreData['inst_chapter_module_id'] 
        = window.ODSA.UTILS.getChapterModuleID();
      scoreData['inst_section_id'] 
        = window.ODSA.TP.instSectionId || data.state.inst_section_id;
    } else if (window.ODSA.UTILS.isStandaloneModule()) {
      scoreData['inst_module_version_id'] 
        = window.ODSA.UTILS.getInstModuleVersionId();
    }
    else {
      scoreData['inst_course_offering_exercise_id'] 
        = window.ODSA.UTILS.getInstCourseOfferingExerciseId();
    }

    scoreData['inst_book_section_exercise_id'] 
      = window.PARAMS.instBookSectionExerciseId 
        || data.state.inst_book_section_exercise_id

    console.log("SENDING TO SERVER: ", scoreData);

    jQuery.ajax({
      url: "/odsa_exercise_attempts/new",
      type: "POST",
      data: JSON.stringify(scoreData),
      contentType: "application/json; charset=utf-8",
      datatype: "json",
      ...
      success: function (data) {
        ...
        //localStorage.removeItem(key);
          ...
          const response = {
            subject: 'SPLICE.reportScoreAndState Response',
            status: 'success',
            exercise_progress: data
          };
