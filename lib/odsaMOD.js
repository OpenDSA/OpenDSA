...
(function ($) {
  ...
  function processEventData(data) {
    if (!data.av) {
      console.warn('Received data without av:', data);
      return; // Skip processing if av is undefined
    }
    var flush = false;
    ...
    console.log('data.av is set', data.av);
  ...
  $(document).ready(function () {
    ...
    console.log("Module name initialized to:", moduleName);
    ...
    $('.embedContainer, .ssAV, .ffAV').each(function (index, elem) {
      var $elem = $(elem),
        ...
        console.log("Processing exercise:", exerName);
      ...
      if (exercises[exerName] || $elem.hasClass('hideable_content_container')) {
        console.log('Exercise already initialized or hideable:', exerName);
        return;
      }
      ...
      console.log('Exercise initialized:', exercises[exerName]);
    });

    // Listen for and process JSAV events
    $("body").on("jsav-log-event", function (e, data) {
      console.log("JSAV event received:", data);
    ...
    function initializeExerciseIfMissing(avName) {
      if (!exercises[avName]) {
        console.warn('Exercise data missing for:', avName, '- initializing with defaults.');
        exercises[avName] = {
        ...
        console.log('Exercise initialized with default settings:', exercises[avName]);
      }
    }
    ...
    eventer(messageEvent, function (e) {
      ...
      if (data) {
        ...
        if (data.subject === 'lti.frameResize') {
          console.log('Handling lti.frameResize for:', data.exerName);
        ...
        if (data.subject === 'SPLICE.reportScoreAndState') {
          console.log('Handled SPLICE.reportScoreAndState:', data)
            return;
        }
        if (data.subject === 'SPLICE.sendEvent') {
          console.log('Handled SPLICE.sendEvent:', data);
            return;
        }

        if (data.type === "resize-iframe") {
          console.log('Handling resize-iframe for:', data.exerName);
          var $iframe = $('#' + data.exerName + '_iframe');
          ...
          console.log("Processing data for av:", data.av);
          if (exercises[data.av]) {
            console.log('Found exercise data for av:', data.av, exercises[data.av]);
