...
(function ($) {
  ...
  function ltiIframeResize() {
    // resize iframe for all sections except Glossary
    if (ODSA.SETTINGS.MODULE_NAME !== 'Glossary' && odsaUtils.scoringServerEnabled()) {
      try {
        ...
        try {
          ...
          // postMessage seems to silently fail since canvas got hacked
          // the iframe is now proxied somehow?
        }

        // we can hack the css to insert a scrollbar as a half solution
        $('html, body').css({ 'overflow-y': 'auto', 'height': 'auto', 'min-height': '2000px' });
        $('.content').css({ 'overflow-y': 'auto', 'max-height': 'none' });
      } catch (e) {
        console.warn('ltiIframeResize error:', e);
      }
    }
  }
