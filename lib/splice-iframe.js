...
if (window.self !== window.top) { // iframe
  // Activities normally load this file synchronously, before the load
  // event fires. However, when it is loaded asynchronously (e.g. via
  // requirejs, as done for KA exercises), the load event may already
  // have fired by the time this runs — so run the initializer directly
  // in that case instead of registering a listener that never fires.
  const spliceInit = event => {
    const scores = {};
    const states = {};
    let callbacks = [];
    let docHeight = 0;
    ...
    const sendScoreData = ({location, score, state}) => {
      // Only meaningful when OpenDSA framework libs are present (AV/exercise
      // pages loaded with odsaUtils). Other embedded activities (e.g. KA
      // exercises loaded via khan-exercises) have no window.ODSA — skip
      // silently instead of throwing.
      if (!window.ODSA || !window.ODSA.UTILS || !window.ODSA.UTILS.scoringServerEnabled()) { return ; }
      ...
      const scoreData = {
        ...
        uiid:        +new Date(),
        type:        window.ODSA.UTILS.getType()
      };
      ...
      jQuery.ajax({
        ...
        success: function (data) {
          console.log("Succesfully Created Attempt Record");
        }, 
        error: function (data) {
    ...
    if (!("SPLICE" in window)) {
      window.SPLICE = {
        ...
        reportScoreAndState: (location, score, state) => {
          ...
          // Also log event in user_interactions table (only in OpenDSA
          // framework contexts; plain embedded activities have no ODSA)
          message.type = "SPLICE.reportScoreAndState";
          message.desc = {location, score, state};
          if (window.ODSA && window.ODSA.UTILS) window.ODSA.UTILS.logEvent(message);
        },
  ...
  if (document.readyState === "complete") {
    ...
    window.addEventListener("load", spliceInit);
  }
}
