function KnowledgeMapInitGlobals() {
    window.KnowledgeMapGlobals = {
        ...
        icons: {
                Exercise: {
                        Proficient: "../build/Images/node-complete.png?" ,
                        Review: "../build/Images/node-review.png?" + 2,
                        Suggested: "../build/Images/node-suggested.png?" + 2,
                        Normal: "../build/Images/node-not-started.png?" 
                          },
                Summative: {
                        Normal: "/static/images/node-challenge-not-started.png?" + 2,
                        Proficient: "/static/images/node-challenge-complete.png?" + 2,
                        Suggested: "/static/images/node-challenge-suggested.png?" + 2
                           }
        },
    ...
    window.KnowledgeMapExercise = Backbone.Model.extend({
        initialize: function() {
            ...
            if (this.get("status") == "Suggested") {
                this.set({"isSuggested": true, "badgeIcon": "../build/Images/" + s_prefix + "-suggested.png?" + KA_VERSION});
            } else if (this.get("status") == "Review") {
                this.set({"isSuggested": true, "isReview": true, "badgeIcon": "../build/Images/node-review.png?" + KA_VERSION});
            } else if (this.get("status") == "Proficient") {
                this.set({"isSuggested": false, "badgeIcon": "../build/Images/" + s_prefix + "-complete.png?" + KA_VERSION});
            } else {
                this.set({"isSuggested": false, "badgeIcon": "../build/Images/" + s_prefix + "-not-started.png?" });
            }
        ...
        url: function() {
            return "../build/" + this.get("name")+".html";  //until we find a way to create a build folder in sitestatic
        },
    ...
    this.drawMarker = function(node, zoom) {
        ...
        var marker = new com.redfin.FastMarker(
                ...
                [   "<a data-id='" + node.name + "' class='nodeLabel'>" +
                    ...
                    "<img class='exercise-goal-icon' style='display: none' src='../build/Images/flag.png'/>" +
