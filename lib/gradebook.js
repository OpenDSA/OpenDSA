...
(function ($) {
  ...
  */
        ...
        if (modExercises.hasOwnProperty(exercise) && typeof modExercises[exercise].points !== "undefined") {
          ...
          if (!modExercises[exercise].hasOwnProperty('long_name')) {
            ...
            if (modExercises[exercise].points === 0) {
              var obj_Exe = getObjects(chapters, 'long_name', modExercises[exercise].exercise);
              if (typeof obj_Exe[0].points !== "undefined"){
  ...
  function generateGradeTable() {
    ...
    html = '<table class="data" width="70%"><tr><th>Chapter</th><th class="score">Score</th></tr>' + html;
    ...
    html1 = '<table class="data" width="70%"><tr><th>Assignment (Due Date)</th><th class="score">Score</th></tr>' + html1;
    html1 += '<tr class="header"><th>Total</th><th class="score">' + data1.userTotal.toFixed(2) + ' / ' + data1.total.toFixed(2) + '</th></tr></table>';
  ...
  function loadGradebook() { // Queries the server for the user's points
    if (settings.DEBUG_MODE) {
      ...
      console.debug('book: ' + settings.BOOK_NAME);
    }
