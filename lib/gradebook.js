...
(function ($) {
    ...
    if (Object.keys(modExercises).length === 0) { // Module contains no exercises
      ...
      for (var exercise in modExercises) {
        if (modExercises.hasOwnProperty(exercise) && typeof modExercises[exercise].points !== "undefined") {
          ...
          html += '><td><a href="' + modName + '.html#' + exercise + '_exer">' + modExercises[exercise].long_name + '</a></td>';

          if (ODSA.MOD.getProficiencyStatus(exercise) === Status.STORED) {
  ...
  function generateSection(secData, prefix, depth) {
    ...
    for (var sect in secData) {
      if (secData.hasOwnProperty(sect)) {
        ...
        html += (ODSA.MOD.getProficiencyStatus(sect) === Status.STORED) ? '<td class="score proficient">' : '<td class="score">';
  ...
  Gradebook.load = function () { // Queries the server for the user's points
    if (settings.debugMode) {
      ...
      console.debug('book: ' + settings.bookName);
    }
    ...
    if (odsaUtils.serverEnabled() && odsaUtils.userLoggedIn()) {
      ...
      jQuery.ajax({
        ...
        type:  "POST",
        data: {"key": odsaUtils.getSessionKey(), "book": settings.bookName},
        ...
        success: function (data) {
          ...
          if (data.grades && data.modules) {
            ...
            for (var i = 0; i < data.grades.length; i++) {
              ...
              ODSA.MOD.storeProficiencyStatus(name, (data.grades[i].points > 0) ? Status.STORED : false, username);
            }

            // Load proficiency data for modules
            for (i = 0; i < data.modules.length; i++) {
              name = data.modules[i].module;
              ODSA.MOD.storeProficiencyStatus(name, (data.modules[i].proficient) ? Status.STORED : false, username);
            }
