...
(function () {
  ...
  Khan.buildAttemptData = function (correct, attemptNum, attemptContent, timeTaken, skipped, optOut) {
    ...
    // handler for SPLICE protocol
    if (typeof window.SPLICE !== 'undefined' && window.SPLICE.reportScoreAndState && window.SPLICE.sendEvent) {
      var exerciseName = data.sha1 ? data.sha1 : 'unknown';
      data.isKA = true;
      window.SPLICE.reportScoreAndState(exerciseName, data.complete, data);
      window.SPLICE.sendEvent(exerciseName, 'attemptData', data);
    } else {
      console.warn("SPLICE is not defined in this exercise.");
    }
    return data;
  };
  ...
  Khan.saveAttemptToServer = function (url, attemptData) {
    ...
    window.addEventListener("message", function (e) {
      ...
      if (data.subject !== 'SPLICE.reportScoreAndState Response') { return; }

      console.log("KA RECIVED: ", data)
      console.log("KA RECIVED: ", data.exercise_progress.exercise_progress)
      ...
      if (data.status === "success") {
        ...
        if (data) {
          current_score = data.exercise_progress.exercise_progress.current_score;
          highest_score = data.exercise_progress.exercise_progress.highest_score;
          threshold = Number(data.exercise_progress.exercise_progress.threshold);
        }
