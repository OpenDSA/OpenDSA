...
(function() {
  var exerciesPath = window.location.pathname,
    ...
    scoringServerEnabled = query.scoringServerEnabled,
    instBookId = query.instBookId ? query.instBookId : null,
    instSectionId = query.instSectionId ? query.instSectionId : null,
  ...
  Khan.odsaGetExerciseData = function() {
    ...
    if (scoringServerEnabled) {
      ...
      jQuery.ajax({
        ...
        // url: "/odsa_exercise_progresses?exerciseName=" + exerciseName,
        url: "/odsa_exercise_progresses/" + instBookId + "/" + instSectionId + "/" + exerciseName,
        type: "GET",
        ...
        success: function(data) {
          ...
          if (data.exercise_progress) {
            var current_score = parseInt(data.exercise_progress.current_score),
          ...
          Khan.odsaPointsAreaPromise.done(function() {
            ...
            var points_progress = $("<span id = 'points-progress' style = 'font-size : 65%; font-weight : bold;'></span>").text(current_score);
  ...
  Khan.buildAttemptData = function(correct, attemptNum, attemptContent, timeTaken, skipped, optOut) {
    ...
    _.extend(data, {
      key: SESSION_KEY,
      inst_book_id: instBookId,
      inst_section_id: instSectionId,
  ...
  Khan.saveAttemptToServer = function(url, attemptData) {
    ...
    promise.done(function(data) {
      ...
      if (highest_score >= threshold || current_score >= threshold) {
        current_progress = threshold;
        $('#prof_check_mark').show();
      } else {
        current_progress = current_score;
