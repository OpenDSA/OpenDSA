...
(function () {
  var exerciesPath = window.location.pathname,
    ...
    jsavExCSS = ["ArrayTree", "DoubleLinkList", "huffman", "PRquadtree", "SkipList"],
    toolProviderData = {};
  scoringServerEnabled = (query.scoringServerEnabled === "true");
  loggingServerEnabled = (query.loggingServerEnabled === "true");

  //set module origin with option of using post message (lightweight/splice protocol use case)
  //if there are any issues with module origin, comment line 21 - 53 and uncomment the original line 12
  var MODULE_ORIGIN = "";
  try {
    MODULE_ORIGIN = parent.location.protocol + '//' + parent.location.host;
  } catch (e) {
    console.warn("Can't set MODULE_ORIGIN using parent.location.", e); //only happens when the exercises run outside of OpenDSA
  function requestParentOrigin() {
    return new Promise((resolve, reject) => {
      window.parent.postMessage({ type: 'request-origin' }, '*');
      // Listen for the response message from the parent
      window.addEventListener('message', function(event) {
        if (event.data.type === 'response-origin') {
          MODULE_ORIGIN = event.data.origin;
          console.log("MODULE_ORIGIN set with post message:", MODULE_ORIGIN);
          resolve(MODULE_ORIGIN);
        }
      });

      setTimeout(() => {
        if (!MODULE_ORIGIN) {
          reject("Failed to receive MODULE_ORIGIN from parent within the expected time.");
        }
      }, 5000);
    });
  }
  requestParentOrigin()
    .catch((error) => {
      console.error(error);
      if (!MODULE_ORIGIN) {
        console.warn("MODULE_ORIGIN could not be determined."); //this is expected when the ka exercises are iframed in an external tool
      }
    });
  }
  // including TP object for LTI 1.3
  if (window.ODSA && window.ODSA.TP) {
    Khan.instBookId = window.ODSA.TP.instBookId || null;
    Khan.instSectionId = window.ODSA.TP.instSectionId || null;
    Khan.instCourseOfferingExerciseId = window.ODSA.TP.instCourseOfferingExerciseId || null;
    Khan.instModuleSectionExerciseId = window.ODSA.TP.instModuleSectionExerciseId || null;
    Khan.instBookSectionExerciseId = window.ODSA.TP.instBookSectionExerciseId || null;
    Khan.instChapterModuleId = window.ODSA.TP.instChapterModuleId || null;
    Khan.instModuleId = window.ODSA.TP.instModuleId || null;
    Khan.instChapterId = window.ODSA.TP.instChapterId || null;
  } else {
    ...
    Khan.instCourseOfferingExerciseId = Khan.query.instCourseOfferingExerciseId
      ? Khan.query.instCourseOfferingExerciseId
      : null;
    Khan.instModuleSectionExerciseId = Khan.query.instModuleSectionExerciseId
      ? Khan.query.instModuleSectionExerciseId
      : null;
    Khan.instBookSectionExerciseId = Khan.query.instBookSectionExerciseId
      ? Khan.query.instBookSectionExerciseId
      : null;
  }
  ...
  Khan.loadCss("../../lib/odsaKA.css", "odsaKA");
    ...
    if (scoringServerEnabled) {
      ...
      if (Khan.instBookId) {
        if (Khan.instBookSectionExerciseId) {
          url = "/odsa_exercise_progresses?inst_book_section_exercise_id=" + Khan.instBookSectionExerciseId;
        }
        else {
        ...
        url = "/odsa_exercise_progresses?inst_module_section_exercise_id=" + Khan.instModuleSectionExerciseId;
      }
      else {
        url = "/odsa_exercise_progresses/" + Khan.instCourseOfferingExerciseId;
      }
      jQuery.ajax({
        ...
        success: function (data) {
          ...
          Khan.odsaPointsAreaPromise.done(function () {
            ...
            var check_mark = $("<span id='prof_check_mark'><img src='../../khan-exercises/images/green_check.png' alt='Proficient'></span>");
  ...
  Khan.loadModule = function (moduleName, exerciseId) {
    ...
    var jsavExtensions = ["ArrayTree", "AVLextension",
      "binaryheap", "Bintree", "CircularQueue",
      "DoubleLinkList", "fp", "huffman", "InfiniteSequence",
      "LinkList", "Plot", "PLutils",
      "PRquadtree", "redblacktree", "SkipList"
    ],
    ...
    if (moduleName === 'jsav' || jsavExtensions.indexOf(moduleName) > -1) {
      require([
        ...
        require(["jsav"],
          function () {
            Khan.loadCss("../../lib/odsaStyle.css", moduleName);
            Khan.loadCss("../../lib/JSAV.css", moduleName);
  ...
  Khan.buildAttemptData = function (correct, attemptNum, attemptContent, timeTaken, skipped, optOut) {
    ...
    // Report the attempt via the SPLICE protocol (fire-and-forget).
    // This is additive: it must NOT replace or block the direct server
    // save in Khan.saveAttemptToServer, which remains the grade-record
    // path. External SPLICE hosts consume these messages; the OpenDSA
    // module page logs them without re-submitting the attempt. Wrapped
    // in try/catch so a reporting failure can never break the attempt
    // pipeline itself (the PR #740 lesson).
    try {
      if (window.SPLICE && window.SPLICE.reportScoreAndState && window.SPLICE.sendEvent) {
        var spliceLocation = Khan.exerciseName || data.problem_type || 'unknown';
        window.SPLICE.reportScoreAndState(spliceLocation, data.complete, {
          type: 'ka-exercise-attempt',
          complete: data.complete,
          count_hints: data.count_hints,
          attempt_number: data.attempt_number,
          time_taken: data.time_taken,
          skipped: data.skipped,
          tstamp: data.client_dt
        });
        window.SPLICE.sendEvent(spliceLocation, 'attemptData', data);
      }
    } catch (e) {
      console.warn('SPLICE reporting failed (attempt still saved normally):', e);
    }
    return data;
  };
  ...
  Khan.saveAttemptToServer = function (url, attemptData) {
    ...
    promise.done(function (data) {
      ...
      if (current_progress >= threshold && MODULE_ORIGIN !== "") {
        ...
        //   if (Khan.instBookSectionExerciseId) {
        //     toolProviderData.instBookSectionExerciseId = Khan.instBookSectionExerciseId;
        //     toolProviderData.exerciseName = Khan.exerciseName;
        //   }
    ...
    promise.fail(function (xhr) {
      ...
      if (error && error.message) {
        $(Exercises).trigger("warning", "ERROR: " + error.message);
        ...
        $(Exercises).trigger("warning",
          "Uh oh, it looks like a network request timed out! " +
          "You'll need to refresh to continue."
        );
      } else {
        $(Exercises).trigger("warning",
          "An error has occured. Try refreshing the page and attempting the exercise again."
        );
      }
  ...
  $(function () {
    ...
    if (loggingServerEnabled && window.TimeMe) {
      // Add timeme.js to track user time on KA exercise
      TimeMe.initialize({
      ...
      // Everytime user comes back reset the exercise time
