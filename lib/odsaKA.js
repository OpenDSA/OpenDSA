      ...
      .done(function(data) {
        ...
        newContents.each(function(index) {
          ...
          if (innerExerciseId.indexOf("Summ") > -1) {
            ...
            subPromises.push(Khan.loadSummExercise(innerExerciseId, fileName));
          }
        });
      ...
      if (data && typeof OpenPopKa !== "undefined") {
        ...
        if (data.correct) {
          ...
          if (data.message !== null) {
            ...
            // if ($("#progfeedback").length == 0) {
            //   var $progfeedback = $("<div>").attr('id', 'progfeedback').attr('title', 'Errors');
            //   $('#codeTextarea').prepend($progfeedback);
            // }

            // $("#progfeedback").dialog({
            //   open: function(event, ui) {
            //     var vDlg = $(event.target).parent();
            //     var vCont = $('#page-container');
