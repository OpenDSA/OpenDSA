...
(function() {
  ...
  Khan.queryEx = function() {
      var jsonData = {};
      jsonData.book = BOOK_ID;
      jsonData.module = MODULE_NAME;
      jsonData.key = SESSION_KEY;
      jsonData.exerciseName = exerciseName;
      ...
      if (server !== null && SESSION_KEY !== 'phantom-key') {
        ...
        jQuery.ajax({
          // Do a request to the server API
          url: server + "/api/v1/user/exercise/queryexercise/",
          type: "GET",
          data: jsonData,
          dataType: "json",
          async: false,
  ...
  Khan.saveAttemptToServer = function(url, attemptData) {
    if (server == null || (attemptData.attemptNum === -1 && (OpenPopKa !== 'undefined'))) {
      return;
    }
