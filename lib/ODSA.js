...
function sendEventData() {
  if (serverEnabled()) {
    ...
    var eventList = getJSON(localStorage.event_data);
    ...
    json_data.actions = JSON.stringify(eventList);    // TODO: Find a better way to send this list so that Django can still interpret it
    ...
    jQuery.ajax({
      ...
      error: function (data) {
        ...
        if (data.status === 400) {    // Bad request
          ...
          if (inLocalStorage('event_data')) {
            var newData = json_data.actions.replace(']', ',') + localStorage.event_data.replace('[', '');
            localStorage.event_data = newData;
          } else {
            localStorage.event_data = JSON.stringify(eventList);
          }
        }
      }
    });
