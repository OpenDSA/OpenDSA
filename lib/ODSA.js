"use strict";
/*global alert: true, console: true, warn: true, checkModuleProficiency, getModuleName */
...
var server_url = "http://opendsa.cc.vt.edu";
...
//  - uid - a unique instance identifier which allows events to be tied to an instance of an exercise
...
// Constants used for tracking the status of an exercise
 ...
 */
function storeProficiencyStatus(exerName, status) {
  status = (status) ? status : STORED;

  var username = getUsername(),
      profData = {},
      exerData = {'status': status};
  
  if (exercises[exerName] && exercises[exerName].points) {
    exerData.points = exercises[exerName].points;
  }
  ...
  profData[username][exerName] = exerData;
 ...
 */
function updateExerProfDisplay(exerName) {
  var profData,
      status = false;
  
  // Get the status from the local proficiency store
  if (inLocalStorage("proficiency_data")) {
    profData = getJSON(localStorage.proficiency_data);
    
    if (profData[username] && profData[username][exerName]) {
      status = profData[username][exerName].status;
    }
  }

  var warning = $('#' + exerName + '_prof_warning');
  var check = $('#' + exerName + '_check_mark');

  // Hide or display proficiency check mark, if it exists
  if (check.length > 0) {
    if (status === SUBMITTED) {
      // Display the proficiency check mark and warning
      check.show();
      warning.show();
  ...
  var btn = $('#' + exerName + '_showhide_btn');
...
function buttonLogger() {
  ...
  if (serverEnabled()) {
    ...
    //console.debug("buttonLogger: (" + AV_NAME + ", " + type + ", " + desc + ")");  // FOR TESTING

    logUserAction(AV_NAME, type, desc);
  }
}
...
/*jslint validthis: true */
  if (serverEnabled()) {
    ...
    //console.debug("linkLogger: (" + AV_NAME + ", " + type + ", " + desc + ")");  // FOR TESTING
  }
}
...
function sendEventData() {
  if (serverEnabled()) {
    ...
    json_data.actions = eventStr;    // TODO: Find a better way to send this list so that Django can still interpret it
    ...
    jQuery.ajax({
      ...
      success: function (data) {
        ...
        if (!data.success) {
          console.warn('Event data rejected by server');
          console.log(json_data);
        }
      },
      error: function (data) {
        data = getJSON(data);
        console.warn("Error sending event data");
        console.log(data);

        if (data.status === 400) {    // Bad request
          // Server rejected log data, discard the rejected
          // log data so it doesn't cause a backlog
          localStorage.removeItem('event_data');
        } else {
  ...
  if (serverEnabled()) {
    ...
    if (userLoggedIn()) {
      ...
      jQuery.ajax({
        ...
        error: function (data) {
          data = getJSON(data);
          console.warn("Error sending score data");
          console.log(data);
...
$(document).ready(function () {
  ...
  // Add all exercises that have a showhide button
  $('.showHideLink').each(function (index, item) {
    var exerName = $(item).attr('id').replace('_showhide_btn', ''),
        exerData = {};
    
    exerData.type = 'pe';
    exerData.uid = +new Date();
    exerData.threshold = $(item).data('threshold');
    exerData.required = $(item).data('required');
    exerData.points = $(item).data('points');
    
    // If the exercises is a Khan Academy exercise add it to a special list and change the exercise type
    if ($(item).data('frame-src') && $(item).data('frame-src').indexOf('Exercises') > -1) {
      kaExerList.push(exerName);
      exerData.type = 'ka';
    }
  ...
  // Add all iframe that are not already in the list
  $('iframe').each(function (index, item) {
    var exerName = $(item).attr('id').replace('_iframe', '');
  ...
  // Add all slideshows that are not already in the list
  $('.ssAV').each(function (index, item) {
    var exerName = $(item).attr('id');
  ...
  if (serverEnabled()) {
    ...
    $("body").on("jsav-log-event", function (e, data) {
      ...
      if (AV_NAME !== '') {
        // This is an AV page, use AV_NAME
        data.av = AV_NAME;
      } else {
        // This event is from a slideshow on a module page, parse the AV name from the ID returned by the JSAV event
      ...
      if (ssEvents.indexOf(data.type) > -1) {
        ...
        // Store completion credit
        storeExerciseScore(data.av, "pe", score);
        flushStoredData();
    ...
    $("body").on("jsav-log-event", function (e, data) {
      ...
      if (data.type === "odsa-award-credit") {
        updateExerProfDisplays(data.av, true);
      }
    });
  }
