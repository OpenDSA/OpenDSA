"use strict";
/*global alert: true, console: true, warn: true */
// These following are local functions, but circular dependancies force
// action to make jshint validate
/*global log_user_action, warn_user_login*/
...
/**
 * Overrides the array toString() function
 */
 ...
 */
function storeProficiencyStatus(avName) {
  ...
  if (inLocalStorage("proficiency_data")) {
    ...
    if (data[username]) {
      ...
      if (av_list.indexOf(avName) === -1) {
        av_list.push(avName);
      }
    } else {
      // Add a new entry for the user
      data[username] = [avName];
    }
  } else {
 ...
 */
function updateProfDisplay(avName, status) {
  var objId = avName + '_check_mark';
 ...
 */
function checkProfLocalStorage(avName) {
  var username = getUsername();
...
/**
 * Given an exercise name, checks localStorage to see if we have a
...
function checkProficiency(exerName) {
  ...
  updateAllProfDisplays(exerName, profStatus);
  ...
  if (serverEnabled() && userLoggedIn()) {
    // If the exercise is a KA exercise, check the progress
    if (kaExerList.indexOf(exerName) > -1) {
      jQuery.ajax({
        url:   server_url + "/api/v1/user/userexercise/getprogress/",
        type:  "POST",
        data: {"username": username, "exercise": exerName },
        ...
        success: function (data) {
          ...
          if (data.progress) {
            ...
            if ($('#' + exerName + '_iframe')) {
              ...
              //document.getElementById(exerName + '_iframe').contentWindow.updateData(data);
            }
          }
        },
    ...
    jQuery.ajax({
      ...
      success: function (data) {
        ...
        if (data.proficient) {
          updateAllProfDisplays(exerName, true);
        }
      },
      error: function (data) { console.log("ERROR " + "isProficient" /*JSON.stringify(data)*/); }
    });
 ...
 */
function log_user_action(avName, type, desc) {
  if (serverEnabled()) {
    console.log("log_user_action(" + avName + ', ' + type + ', ' + desc + ')');
    var json_data = {};
    json_data.av = avName;
    json_data.type = type;
    json_data.desc = desc;
 ...
 */
function log_exercise_init(avName, initData) {
  if (serverEnabled()) {
    log_user_action(avName, 'exercise_initialization', JSON.stringify(initData));
  }
}
...
/*jslint validthis: true */
  if (serverEnabled()) {
    ...
    if (this.id !== "") {
      ...
      console.log(this.value + " button does not have an ID");
    }
    ...
    console.log("button_logger: (" + avName + ", " + type + ", " + desc + ")");

    log_user_action(avName, type, desc);
  }
}
...
/*jslint validthis: true */
  if (serverEnabled()) {
    ...
    if (this.id !== "") {
      ...
      console.log("Link (" + this.href + ") does not have an ID");
    }
    ...
    console.log("link_logger: (" + avName + ", " + type + ", " + desc + ")");  // FOR TESTING
  }
}
 ...
 */
function store_av_score(avName, type, score) {
  if (serverEnabled()) {
    var score_data = {};
    ...
    if (userLoggedIn()) {
      ...
      jQuery.ajax({
        ...
        success: function (data) {
          ...
          if (data.proficient) {
            updateAllProfDisplays(avName, true);
          }
        },
        error: function (data) {
...
function awardCompletionCredit(avName) {
  if (serverEnabled()) {
    ...
    store_av_score(avName, "ss", score);
    flush_stored_data();
  ...
  $("body").on("jsav-log-event", function (e, data) {
    ...
    if (ssEvents.indexOf(data.type) > -1) {
      ...
      if ((!serverEnabled() || !userLoggedIn()) && data.score.student === data.score.total) {
        ...
        updateAllProfDisplays(data.av, true);
      }
    } else {
      data.desc = data.type;
    }
...
$(document).ready(function () {
  ...
  $('iframe').contents().each(function (index, doc) {
    ...
    if ($('#' + avName + '_showhide_btn').length === 0 && $('#' + avName + '_check_mark', $(doc)).length === 0) {
      console.log('test1: ' + avName + " doesn't have a showhide button");
  ...
  */
  ...
  if (serverEnabled()) {
    ...
    if (isModulePage()) {
      ...
      log_user_action('', 'document-ready', 'User loaded the ' + getModuleName() + ' module page');
      ...
      var avName = getNameFromURL();
      // Log the browser ready event
      log_user_action(avName, 'document-ready', 'User loaded the ' + avName + ' AV');
    }
              ...
              if (inLocalStorage('khan_exercise')) {
                ...
                for (var i = 0; i < kaExerList.length; i++) {
                  ...
                  if ($('#' + kaExerList[i] + '_iframe')) {
                    // TODO: get the correct function to trigger
                    //document.getElementById(kaExerList[i] + '_iframe').contentWindow.updateData(data);
                  }
                }
              }
  ...
  eventer(messageEvent, function (e) {
    ...
    updateAllProfDisplays(msg_ka.exercise, msg_ka.proficient);
  }, false);
});
