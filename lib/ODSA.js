...
function isValidEvent(data) {
  ...
  if (missingFields.length === 1) {
    console.error("Invalid event, '" + missingFields[0] + "' is undefined");
    console.error(data);
    return false;
  } else if (missingFields.length > 1) {
    console.error("invalid event, '" + missingFields.join(', ') + "' are undefined");
    console.error(data);
    return false;
  }
  ...
  if (serverEnabled()) {
    ...
    jQuery.ajax({
      ...
      success: function (data) {
        ...
        // Either the data was successfully saved in which case we can clear it
        // or, the server rejected it because there was something wrong with it, in which case we should clear it
        localStorage.removeItem('event_data');
        
        if (!data.success) {
          console.error('Event data rejected by server');
