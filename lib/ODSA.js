 ...
 *     - If the server responds that the logged in user is proficient
 */
function updateAllProfDisplays(av_name, status) {
...
/*jslint validthis: true */
  if (serverEnabled()) {
    ...
    console.log("button_logger: (" + av_name + ", " + type + ", " + desc + ")");
...
/*jslint validthis: true */
  if (serverEnabled()) {
    ...
    console.log("link_logger: (" + av_name + ", " + type + ", " + desc + ")");  // FOR TESTING
  }
}
    ...
    if (userLoggedIn()) {
      ...
      jQuery.ajax({
        ...
        success: function (data) {
          ...
          if (data.proficient) {
            updateAllProfDisplays(av_name, true);
          }
        },
        error: function (data) {
 ...
 */
function awardCompletionCredit(av_name) {
  if (serverEnabled()) {
    var score = {};
    score.student = 1;
    score.correct = 1;
    ...
    store_av_score(av_name, "ss", score);
    flush_stored_data();
