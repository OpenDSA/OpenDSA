...
(function ($) {
  ...
  if (typeof ODSA === "undefined") {
    ...
    var odsaSettings = {};
    odsaSettings.bookName = getURLParam('bookName');
    odsaSettings.serverURL = getURLParam('serverURL');
    odsaSettings.moduleOrigin = getURLParam('moduleOrigin');
    odsaSettings.debugMode = false;
    
    // If moduleOrigin is not specified, assume they are on the same domain
    if (odsaSettings.moduleOrigin === '') {
      odsaSettings.moduleOrigin = location.protocol + '//' + location.host;
    }
  ...
  if (settings.serverURL !== '' && !settings.serverURL.match(/^https:/)) {
  ...
  function logEvent(data) {
    ...
    if (serverEnabled()) {
      ...
      if (!eventData[settings.bookName]) {
        if (settings.debugMode) {
          console.debug('Creating a new event_data entry for ' + settings.bookName);
        }
        eventData[settings.bookName] = [];
      }

      eventData[settings.bookName].push(data);
  ...
  function sendEventData() {
    ...
    if (serverEnabled()) {
      ...
      if (eventData[settings.bookName]) {
        eventStr = JSON.stringify(eventData[settings.bookName]);
        eventData[settings.bookName] = [];
        localStorage.event_data = JSON.stringify(eventData);
      }
      ...
      eventData.book = settings.bookName;
      ...
      jQuery.ajax({
        url:   settings.serverURL + "/api/v1/user/exercise/avbutton/",
        type:  "POST",
        data: eventData,
        ...
        error: function (data) {
          ...
          if (data.status === 400) {
            ...
            for (var i = 0; i < actions.length; i++) {
              eventData[settings.bookName].push(actions[i]);
            }
  ...
  $(document).ready(function () {
    ...
    localStorage.book = settings.bookName;
