...
/*global logUserAction, warnUserLogin*/
 ...
 *
 * Note: This is designed to only be called by updateExerProfDisplay()
 */
 ...
 */
function updateExerProfDisplay(avName, status) {
 ...
 *     - If the server responds that the logged in user is proficient
 */
function updateExerProfDisplays(avName, status) {
...
function logUserAction(avName, type, desc) {
  if (serverEnabled()) {
    console.log("logUserAction(" + avName + ', ' + type + ', ' + desc + ')');  // FOR TESTING
    var json_data = {};
    json_data.av = avName;
    json_data.type = type;
    json_data.desc = desc;
...
/*jslint validthis: true */
  if (serverEnabled()) {
    ...
    console.log("buttonLogger: (" + avName + ", " + type + ", " + desc + ")");  // FOR TESTING
...
/*jslint validthis: true */
  if (serverEnabled()) {
    ...
    console.log("linkLogger: (" + avName + ", " + type + ", " + desc + ")");  // FOR TESTING
  }
}
...
function awardCompletionCredit(avName) {
  if (serverEnabled()) {
    ...
    storeExerciseScore(avName, "ss", score);
    flushStoredData();
  ...
  */
  ...
  if (serverEnabled()) {
    ...
    if (isModulePage()) {
      ...
      logUserAction('', 'document-ready', 'User loaded the ' + getModuleName() + ' module page');
