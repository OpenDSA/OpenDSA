...
(function ($) {
  ...
  function logEvent(data) {
    ...
    if (serverEnabled()) {
      ...
      data.book = settings.BOOK_NAME;;  // TODO: Send the book identifier
      ...
      // TODO: Do events need to be tied to a specific book? aka include book URL
      ...
      localStorage['event-' + data.tstamp + '-' + rand] = JSON.stringify(data);
    }
  ...
  function sendEventData() {
    ...
    if (serverEnabled()) {
      ...
      for (var key in localStorage) {
        //if (key.startsWith('event-') && parseInt(key.split('-')[1]) < tstamp) {
        if (key.indexOf('event-')===0 && parseInt(key.split('-')[1]) < tstamp) {
