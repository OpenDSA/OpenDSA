...
(function ($) {
  ...
  // Copy exists in odsaAV.js (if updates are made be sure to update the copy as well)
  /**
  ...
  if (!!settings.SCORE_SERVER && !settings.SCORE_SERVER.match(/^https:/)) {
   ...
   */
  function getBookID() {
    // Use the BOOK_ID supplied via URL parameter if its available, otherwise take the SHA1 hash of the book's URL
