...
(function($) {
  ...
  function loadConfig(args) {
    ...
    if (args.hasOwnProperty('json_path')) {
      if (loggingServerEnabled()) {
        json_url = "/OpenDSA" + args.json_path;
      } else {
  ...
  /**
   * Enable text-to-speech if specified by book config
   */
  if (settings.NARRATION_ENABLED) {
    window.JSAV_OPTIONS.narrationEnabled = true;
  }
  else {
    window.JSAV_OPTIONS.narrationEnabled = false;
  }
  ...
  odsaUtils.ssEvents = ['jsav-forward', 'jsav-backward', 'jsav-begin', 'jsav-end', 'jsav-exercise-model-forward', 'jsav-exercise-model-backward', 'jsav-exercise-model-begin', 'jsav-exercise-model-end', 'jsav-narration-enable', 'jsav-narration-disable'];
