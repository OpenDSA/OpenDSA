...
(function($) {
  ...
  function loadLangMod() {
    var langText = {},
      ...
      url = '/Books/' + ODSA.SETTINGS.BOOK_NAME + '/html/_static/language_msg.json';
  ...
  function loadConfig(args) {
    ...
    if (args.hasOwnProperty('json_path')) {
      ...
      if (loggingServerEnabled()) {
        json_url = "/OpenDSA/" + json_url
      }

      // if (settings.hasOwnProperty('BUILD_TO_ODSA')) {
      //   if (loggingServerEnabled()) {} else {
    ...
    if (result.code) {
      ...
      if (Array.isArray(result.code)) {
        for (var i = 0; i < result.code.length; i++) {
          ...
          result.code[i].url = '/' + url.slice(url.indexOf("SourceCode"));
          if (loggingServerEnabled()) {
            result.code[i].url = "/OpenDSA" + result.code[i].url;
          }
        }
      } else {
        url = result.code.url;
        result.code.url = '/' + url.slice(url.indexOf("SourceCode"));
        if (loggingServerEnabled()) {
          result.code.url = "/OpenDSA" + result.code.url;
        }
      }
    }
