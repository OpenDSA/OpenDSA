...
(function ($) {
   ...
   */
  function getInterpreter(av_name, av_container) {
    if (typeof av_name === 'undefined') {
      av_name = settings.AV_NAME;
    }

    if (typeof av_container === 'undefined') {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (typeof configurationFile !== 'undefined' && configurationFile.hasOwnProperty('translations')) {
          ...
          interpreter = JSAV.utils.getInterpreter(translation);
  ...
  odsaUtils.getInterpreter = getInterpreter;
