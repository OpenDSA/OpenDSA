...
(function ($) {
  ...
  function loadConfig(args) {
    ...
    // add odsa prefix depending on deployment environment
      if (location.origin.includes("opendsax") ||
          location.origin.includes("opendsa-server.cs.vt.edu")){
          json_url = "/ODSA"+json_url
      }
  ...
  */
  ...
  $("body").on("jsav-log-event", function (event, eventData) {
    if (eventData.av === 'container') {
        // Delaying SPLICE.sendEvent until 'av' is updated.
        setTimeout(function() {
            if (eventData.av !== 'container') {
                // console.log("Delayed event handling - 'av' is now updated:", eventData.av);
                sendSpliceEvent(eventData);
            }
        }, 100);
    } else {
        sendSpliceEvent(eventData);
  ...
  // handle sending SPLICE events
  function sendSpliceEvent(eventData) {
      if (['jsav-exercise-grade', 'jsav-exercise-grade-change', 'jsav-exercise-step-fixed', 'jsav-exercise-gradeable-step', 'jsav-array-click'].includes(eventData.type)) {
          if (window.SPLICE && window.SPLICE.sendEvent) {
              if (eventData.type === 'jsav-exercise-grade' || eventData.type === 'jsav-exercise-grade-change') {
