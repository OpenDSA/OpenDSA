...
(function($) {
  ...
  function getBookID() {
    if (typeof ODSA.SETTINGS.BOOK_ID === "undefined") {
      var launch_params = ODSA.TP.toParams.launch_params;
      var canvas_course_id = launch_params.custom_canvas_course_id;
      var canvas_instance_url = launch_params.lis_outcome_service_url;

      // parse the hostname
      var a = $('<a>', {
        href: canvas_instance_url
      })[0];

      ODSA.SETTINGS.BOOK_ID = Sha1.hash(a.hostname + '-' + canvas_course_id);
    }
  ...
  function userLoggedIn() {
    ...
    return !isSessionExpired() && (userId === localStorage.username);
  }
    ...
    if (loggingServerEnabled()) {
      ...
      for (var key in localStorage) {
        ...
        if (key.indexOf('event-') === 0 && parseInt(key.split('-')[1], 10) < tstamp) {
          ...
          if (eventObj.user == localStorage.username && eventObj.book == getBookID()) {
