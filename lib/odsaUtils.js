...
(function () {
  ...
  */
  ...
  function logTimeTrackingData(key, currentPageName, trackingPageNames, odsaStore) {
    ...
    if (hasBook()) {
      ...
      if (isFullModule()) {
        logData['inst_chapter_module_id'] = getChapterModuleID();
      }
      else {
  ...
  *                  the embedded AV.
  */
  function InitializeTimeMe(chapterName, moduleName, moduleSections, exerciseName) {
    var currentPageName = '';
    if (moduleSections) {
      currentPageName = chapterName.toLowerCase() + "-" + moduleName.toLowerCase();
    } else {
  ...
  $(function () {
    ...
    if (hasBook() || isStandaloneModule()) {
      var odsaStore = InitializeTimeMe(chapterName, moduleName, moduleSections, exerciseName)
  ...
  // Add publicly available functions to a globally accessible ODSA.UTILS object
  var odsaUtils = {};
