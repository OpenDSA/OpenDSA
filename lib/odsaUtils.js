...
(function ($) {
  ...
  /**
   * Loads a JSON file that contains language and code translations, attempts to
   * reset the language of common elements, returns an object that contains a
   * JSAV interpreter and the code
   *
   * Parameters:
   ...
   *
   *  - js_filename - the name of the JS file containing the code for the AV,
   *                  loadLangData() will search for an included JS file with this
   *                  name to determine where to look for the JSON file
   *                  (used to auto-detect the location of the JSON file)
   *
   *
   *  - json_filename - the name of the JSON file containing language data,
   *                    defaults to av_name (convention for standalone AVs)
   *
   ...
   *   - Ex: {interpreter: ..., code: ...}
   */
  function loadLangData(args) {
    // Initialize args, if no object was provided
    if (typeof args === 'undefined') {
      args = {};
    }

    // Default av_name to ODSA.SETTINGS.AV_NAME (valid for standalone AVs)
    var av_name = settings.AV_NAME;

    // Overrides default settings, using provided arguments (necessary for mini-slideshows)
    if (args.hasOwnProperty('av_name')) {
      av_name = args.av_name;
    }

    // Assume the JS file with the AV code and the JSON language fil
    // have the same name as the AV (convention for standalone AVs)
    var js_filename = av_name;
    var json_filename = av_name;

    if (args.hasOwnProperty('js_filename')) {
      js_filename = args.js_filename;
    }
    if (args.hasOwnProperty('json_filename')) {
      json_filename = args.json_filename;
    }

    // Append '.json' to the end of the JSON filename, if it doesn't already exist
    if (json_filename.indexOf('.json') === -1) {
      json_filename += '.json';
    }

    // Default json_path to json_filename (convention for standalone AVs)
    var json_url = json_filename;
    ...
    if (args.hasOwnProperty('av_container') && $('#' + args.av_container).length > 0) {
      ...
      // If the container is named for the AV it is a mini-slideshow and we should compute the json_path
      // Find the script tag whose src contains the name of the JS file where the AV is defined, get the src and append a 'on' to form '.json'
      if (js_filename.indexOf('.js') === -1) {
        js_filename += '.js';
      }

      // Find the path to the JS file, then replace the JS filename with the JSON filename
      json_url = $('script[src*="/' + js_filename + '"]')[0].src.replace(js_filename, json_filename);
    ...
    var code_lang = (JSAV_EXERCISE_OPTIONS.code || JSAV_OPTIONS.code || '').toLowerCase();
      ...
      success: function (data) {
        ...
        if (configurationFile.hasOwnProperty('params')) {
          for (key in configurationFile.params) {
            // Allow the configuration file to apply defaults
