...
(function ($) {
  ...
  function hasToolProvider() {
    return typeof ODSA.TP !== "undefined";
  }
  ...
  function loadConfig(args) {
    ...
    if (args.hasOwnProperty('json_path')) {
      ...
      if (loggingServerEnabled()) {
        ...
        // add prefix depending on deployment environment
        if (location.origin.includes("opendsa")) {
          json_url = "/OpenDSA/" + fixed;
        } else {
          json_url = fixed;
	}
  ...
  //sends user interaction data to DataShoplogger
  // function sendDataToDataShop(userInteractions) {
  //   jQuery.ajax({
  //     url: "https://datashop-logging.localhost.devcom.vt.edu/odsa_user_interactions", //to be reeplaced
  //     type: "POST",
  //     data: JSON.stringify(userInteractions),
  ...
  //     },
  //     success: function (data) {
  //       console.log("Data successfully sent to DataShop");
  //     },
  //     error: function (data) {
  //       console.group("Error sending data to DataShop");
  //       console.debug(data);
  //       console.groupEnd();
  //     }
  //   });
  ...
  // function sendDataToDataShop(userInteractions) {
  //   jQuery.ajax({
  //     url: "https://datashop-logging.localhost.devcom.vt.edu/odsa_user_interactions",
  //     type: "POST",
  //     data: JSON.stringify(userInteractions),
  //     contentType: "application/json; charset=utf-8",
  //     datatype: "json",
    ...
    if (loggingServerEnabled()) {
      ...
      jQuery.ajax({
        ...
        success: function (data) {
          ...
          //callback for Datashop function
          // sendDataToDataShop(userInteractions);
  ...
  function sendTimeTrackingData(timeTrackingData, localforage) {
    ...
    for (const property in modulesTracking) {
      ...
      allTimes = allTimes.map(function (item) {
      ...
      modulesTracking[property]["allTimes"] = allTimes;
    ...
    return retryPostAPICall("/odsa_user_time_tracking", data)
      //retryPostAPICall("https://datashop-logging.localhost.devcom.vt.edu/odsa_user_interactions", data)
      .then(() => {
        var promises = _keys.map(function (item) { return localforage.removeItem(item); });
        return Promise.all(promises);
      })
      .catch((err) => {
        console.log("Failed to send time tracking data", err);
      });
  }
    ...
    return new Promise((resolve, reject) => {
      $.ajax({
        ...
        .done(function (response, _textStatus, jqXHR) {
          resolve({ response: response, status: jqXHR.status });
        })
      ...
      if (retries > 0 && retryCodes.includes(err.status)) {
        return new Promise((resolve) => setTimeout(resolve, backoff)).then(() =>
          retryPostAPICall(url, data, retries - 1, backoff * 2)
        );
      }
  ...
  */
  function InitializeTimeMe(currentPageName, moduleSections, exerciseName) {
    var initChecks = {
      hasTimeMe: !!window.TimeMe,
      hasLocalforage: !!window.localforage,
      loggingEnabled: !!loggingServerEnabled(),
      instBookId: getBookID()
    };
    if (!initChecks.hasTimeMe || !initChecks.hasLocalforage || !initChecks.loggingEnabled || !initChecks.instBookId) {
      return null;
    }
    ...
    if (moduleSections) {
      ...
      // Persist an initial record right away so short visits still create a store entry.
      key = logTimeTrackingData(key, currentPageName, trackingPageNames, odsaStore)
  ...
  function inferModuleSectionsFromDOM() {
    var sections = [];

    function collectSections(selector, headingSelector) {
      document.querySelectorAll(selector).forEach(function (sectionEl) {
        if (!sectionEl.id) {
          return;
        }
        if (sectionEl.id === "exercise-summary-widget") {
          return;
        }
        if (!sectionEl.querySelector(headingSelector)) {
          return;
        }
        if (!sections.includes(sectionEl.id)) {
          sections.push(sectionEl.id);
        }
      });
    }

    collectSections("section[id]", ":scope > h2, :scope > h3");
    collectSections("div.section[id]", ":scope > h2, :scope > h3");
  ...
  $(document).ready(function () {
    ...
    if (!Array.isArray(settings.MODULE_SECTIONS) || settings.MODULE_SECTIONS.length === 0) {
      settings.MODULE_SECTIONS = inferModuleSectionsFromDOM();
    }
    ...
    if (window.TimeMe) {
      window.TimeMe.startTimer(exerciseName, 0);
    }
  ...
  odsaUtils.hasToolProvider = hasToolProvider;
  ...
  // odsaUtils.sendEventDataToDataShop = sendEventDataToDataShop;
  ...
  //*****************************************************************************
  //*******               Listeners & handlers for SPLICE protocol    ***********
  //*****************************************************************************

  // Per-slideshow progress bookkeeping for SPLICE reporting.
  // Mirrors the rules previously applied by the legacy slideshow scoring in
  // odsaMOD.js processEventData(): a slide only counts as viewed when the
  // user steps onto it, and (when REQ_FULL_SS is set) completion requires
  // every slide to have been viewed.
  var spliceSsProgress = {};

  // Report slideshow position, completion, and furthest-slide state
  // through the SPLICE protocol.
  function sendSpliceSlideshowEvent(eventData) {
    if (!(window.SPLICE && window.SPLICE.sendEvent)) {
      console.warn("SPLICE is not defined.");
      return;
    }

    var av = eventData.av;
    if (!av || typeof eventData.currentStep !== "number") { return; }

    var prog = spliceSsProgress[av];
    if (!prog) {
      prog = spliceSsProgress[av] = { furthestStep: 0, startTime: +new Date() };
    }

    // A slide counts as viewed only when the user steps onto it
    if ((eventData.type === "jsav-forward" || eventData.type === "jsav-backward") &&
        eventData.currentStep === prog.furthestStep + 1) {
      prog.furthestStep = eventData.currentStep;
    }

    // Report the current slide position
    window.SPLICE.sendEvent(av, eventData.type, {
      av: av,
      type: eventData.type,
      currentStep: eventData.currentStep,
      totalSteps: eventData.totalSteps,
      furthestStep: prog.furthestStep,
      tstamp: new Date().toISOString()
    });

    // Report completion (only once) when the user reaches the end of the
    // slideshow. If the book requires the full slideshow (REQ_FULL_SS),
    // every slide must have been stepped through, as in the legacy rule.
    var reqFullSS = window.ODSA && ODSA.SETTINGS && ODSA.SETTINGS.REQ_FULL_SS;
    if (window.SPLICE.reportScoreAndState && !prog.complete &&
        eventData.totalSteps > 0 &&
        eventData.currentStep === eventData.totalSteps &&
        (!reqFullSS || prog.furthestStep === eventData.totalSteps)) {
      prog.complete = true;
      window.SPLICE.reportScoreAndState(av, 1, {
        av: av,
        ...
        furthestStep: prog.furthestStep,
        complete: true,
        totalTime: +new Date() - prog.startTime,
        tstamp: new Date().toISOString(),
        uiid: Math.floor(Date.now() / 1000),
        score: { correct: 1, total: 1, undo: 0, fix: 0 }
      });
    }
  }

  $("body").on("jsav-log-event", function (event, eventData) {
      // Adjust AV name to actual exercise iframe name. Only do this when
      // AV_NAME is set (standalone AV/exercise pages) — on book module pages
      // events from inline slideshows already carry their own av name.
      if (ODSA.SETTINGS.AV_NAME) {
        eventData.av = ODSA.SETTINGS.AV_NAME;
      }

      // When an exercise (re)initializes (e.g. the user hits Reset/Run),
      // clear its slideshow progress so position and completion are
      // tracked and reported fresh for the new instance.
      if (eventData.av &&
          (eventData.type === "odsa-exercise-init" ||
           eventData.type === "jsav-init" ||
           eventData.type === "jsav-exercise-init")) {
        delete spliceSsProgress[eventData.av];
      }

      // Slideshow events: report position/completion via the SPLICE protocol
      if (odsaUtils.ssEvents.indexOf(eventData.type) > -1) {
        sendSpliceSlideshowEvent(eventData);
        return;
      }

      const valid_event_types = [
          'jsav-exercise-grade',
          'jsav-exercise-grade-change',
      ...
      if (eventData.type !== 'jsav-exercise-grade' &&
          eventData.type !== 'jsav-exercise-grade-change') {
          window.SPLICE.sendEvent(eventData.av, eventData.type, eventData);
          return;
      }

      if (window.SPLICE.reportScoreAndState) {
          const score = eventData.score ? eventData.score.correct / eventData.score.total : 0;
          window.SPLICE.reportScoreAndState(eventData.av, score, eventData);
      }
  });
