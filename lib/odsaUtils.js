...
(function ($) {
   ...
   *  - json_path - the path to the JSON file containing language data, defaults
   *                to [av_name].json for standalone AVs and
  ...
  function loadLangData(args) {
    ...
    // Default av_name to ODSA.SETTINGS.AV_NAME
    var av_name = settings.AV_NAME;

    // Overrides the default av_name settings, if it was provided as an argument
    if (args.hasOwnProperty('av_name')) {
      av_name = args.av_name;
    }

    // Default json_path to the name of the AV (convention for standalone AVs)
    var json_path = av_name;
    // Default to #container (the av_container for standalone AVs)
    ...
    // Reset json_path if its provided as a parameter
    if (args.hasOwnProperty('json_path')) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (typeof configurationFile !== 'undefined' && configurationFile.hasOwnProperty('translations')) {
          ...
          // If code_lang === 'none', disable code for the AV
          if (code_lang !== 'none') {
