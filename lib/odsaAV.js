...
(function ($) {
  // Set default grading options for JSAV exercises
  window.JSAV_EXERCISE_OPTIONS = { feedback: "continuous", fixmode: "fix" };
  ...
  // Uses arguments passed via the URL to override the default JSAV grading options
  // If individual exercises reset the defaults for JSAV_EXERCISE_OPTIONS, they must call this function to allow configuration
  // Conversely, if an exercise designer does not want their exercise to be configurable, they can reset the defaults without calling this function
  function updateJSAVExerOptions() {
    ...
    for (var i in urlParamList) {
      // Arguments that begin with the prefix 'JXOP-' are JSAV exercise option settings
      if (urlParamList[i].indexOf('JXOP-') === 0) {
        kvPair = urlParamList[i].split('=');
        // Strip the 'JXOP-' flag from the setting name and apply the specified value
        window.JSAV_EXERCISE_OPTIONS[kvPair[0].replace('JXOP-', '')] = kvPair[1];
      }
    }
  }
  
  // Apply an JSAV exercise options passed via the URL
  ...
  av.reset = reset;
  av.updateJSAVExerOptions = updateJSAVExerOptions;
  window.ODSA.AV = av;
  ...
  $(document).ready(function () {
    ...
    $("body").on("jsav-log-event", function (e, data) {
      ...
      } else if (data.type === "jsav-exercise-grade-change" || data.type === "jsav-exercise-grade") {
        // On grade change events, log the user's score and submit it
        data.desc.score = odsaUtils.roundPercent((data.score.correct - data.score.fix) / data.score.total);
