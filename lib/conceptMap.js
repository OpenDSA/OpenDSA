...
(function ($) {
  ...
  function store(id) {
    ...
    myRef = window.open("conceptMap.html", '', simWindowFeatures);
  }
  ...
  function Parser() {
    ...
    var phrase = xmlDoc.getElementsByTagName('linking-phrase');
    ...
    var connection = xmlDoc.getElementsByTagName('connection');
    for(var b = 0; b < connection.length; b++) {
      id = connection[b].getAttribute('id').toLowerCase();
      var from_id = connection[b].getAttribute('from-id').toLowerCase();
      var to_id = connection[b].getAttribute('to-id').toLowerCase();
      connections.push(new Connection(id, from_id, to_id));
    }

    //Concepts or Nodes
    var concept = xmlDoc.getElementsByTagName('concept');
    var id = 0;
    var label = null;
    var definition = null;
    ...
    for(var aa = 0; aa < concept.length; aa++) {
      var thisId = concept[aa].getAttribute('id').toLowerCase();
      var thisLabel = concept[aa].getAttribute('label').toLowerCase();
      var thisDefinition = concept[aa].getAttribute('long-comment');
      ...
      if(isNode(connections[d].from_id)) { 
        ...
        for(var e = 0; e < connections.length; e++) {          
          if(to_id === connections[e].from_id) {
            var toNode = getConceptLabel(connections[e].to_id);
