...
(function() {
  ...
  var blockHeight = 44;  // Width/height of an array or list element
  ...
  function reset() {
    ...
    if ($("#RadixsortPRO")) { $("#RadixsortPRO").empty(); }

    // Set up the display
    av = new JSAV("RadixsortPRO");
    ValueIndex = -1;
    jsavInput = av.ds.array(arrdata, {indexed: true, center: false,
                                      layout: "vertical", left: 20, top: 10});
    ...
    for (i = 0; i < 10; i++) {
      jsavLists[i] = av.ds.list({top: (16 + i * blockHeight), left: 260, nodegap: 30});
      jsavLists[i].layout({center: false});
    }
    av.displayInit();
    av.recorded();

    radixsortPRO.userInput = false;
  }
