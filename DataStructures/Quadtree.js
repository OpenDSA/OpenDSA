	...
	*/
	function QuadTree(jsav, xRange, yRange, notClick ){
		...
		this.qdt = jsav.ds.tree({nodegap:10,left: 50});
		this.qdt.root("");
	...
	QuadTreeProto.insert = function(point, nPoint){
		(nPoint === undefined)? numPoint = 6:numPoint = 2*nPoint;
	...
	leafProto.insert = function (rt, point, x, y, w, h, pointer){
		...
		if (p == 0){
			this.list[this.current++] = this.jsav.g.circle(mapleft + point.getX(), maptop + point.getY(), 2, {fill: 'black'});
	...
	leafProto.split = function(rt, point, x, y, w, h){
		...
		for (var i = 0; i < this.current; i = 0){
			if (noClick){
				...
				this.list[i+1].hide(); // not necessary
			}
