...
console.log("Used block sizes: " + used1Size + ", " + used2Size + ", " + used3Size + ", " + used4Size);
      ...
      for (i = 0; i < 4; i += 1) {
        ...
        for(j = 0; j < 4; j += 1) {
          if(incrs[i] <= modelarr.value(j)) {
            if(modelarr.value(j) < bestSize) {
              bestSize = modelarr.value(j);
              bestIndex = j;
            }
          }
        }
            ...
            if(bestIndex != -1) {
              modelarr.highlight(bestIndex);
              modelarr.unhighlight(bestIndex);
              jsav.stepOption("grade", true);


              var newVal = modelarr.value(bestIndex) - incrs[i];
              modelarr.value(bestIndex, newVal);
    ...
    function about() {
      alert("Best-fit Memory Management Proficiency Exercise\nAuthors: Kanye McKinney, Samantha Fisher, and Cliff Shaffer\nCreated as part of the OpenDSA hypertextbook project\nFor more information, see http://opendsa.org.\nSource and development history available at\nhttps://github.com/OpenDSA/OpenDSA\nCompiled with JSAV library version " + JSAV.version());
    }
