...
console.log("Used block sizes: " + used1Size + ", " + used2Size + ", " + used3Size + ", " + used4Size);
    ...
    function modelSolution(jsav) {
      ...
      for (i = 0; i < 4; i += 1) {
        for(j = 0; j < 4; j += 1) {
          var index = (lastPos + j) % 4;
          if(incrs[i] <= modelarr.value(index)) {

            modelarr.highlight(index);
            modelarr.unhighlight(index);
            jsav.stepOption("grade", true);


            var newVal = modelarr.value(index) - incrs[i];
            modelarr.value(index, newVal);
