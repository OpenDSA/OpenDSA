$(document).ready(function() {
  ...
  var mainVarVal = getRightSideValue(codeLines[currentLineMain]);

  var mainLabel = av.label(mainVarName,
    {
      relativeTo:pseudo, anchor:"right top", myAnchor:"left top",
      left: leftMargin+boxPadding, top: currentTopMargin
    }
  );
  var mainVar = av.ds.array(mainVarVal,
    {
      indexed: mainVarVal.length > 1,relativeTo:mainLabel, anchor:"right top",
      myAnchor:"left top", left: labelMargin,
      top:-1*jsavArrayOffset
    }
  );
  ...
  av.umsg("main's "+mainVarName+" is initialized to "+mainVarVal[0]+".");
...
/*$(document).ready(function() {
  ...
  var CallByAllVal = {
    ...
    init: function() {
      ...
      var values, numDup, i;
      var globalEnv = E.update(E.createEmptyEnv(), ["x", "y", "z"], [E.createNum(1),
        E.createNum(2),
        E.createNum(3)
      ]);
      var tooLong = function(s) {
        return s.length > 3;
      };
...
/*
      ...
      function callByCopyRestoreCallByAllFive(exp, envir) {
        ...
        // make copies
        var copies = args.map(function(arg) {
          return [arg[0]];
        });
      ...
      function evalExpsCallByAllFive(list, envir) {
        return list.map(function(e) {
          return evalExpCallByAllFive(e, envir);
        });
      }
      ...
      function evalExpCallByAllFive(exp, envir) {// ***** Where all the magic happens
          ...
          if (exp.comesFromLetBlock) {
            ...
            switch (SL.ppm) {
              case "byval":
                return callByValueCallByAllFive(exp, envir);
              case "byref":
                return callByReferenceCallByAllFive(exp, envir);
              case "bycpr":
                return callByCopyRestoreCallByAllFive(exp, envir);
              case "bymac":
                return callByMacroCallByAllFive(exp, envir);
              case "bynam":
                return callByNameCallByAllFive(exp, envir);
            }
          }
        } else if (A.isPrimApp1Exp(exp)) {
          return SL.applyPrimitive(A.getPrimApp1ExpPrim(exp), [evalExpCallByAllFive(A.getPrimApp1ExpArg(exp), envir)]);
        } else if (A.isPrimApp2Exp(exp)) {
          return SL.applyPrimitive(A.getPrimApp2ExpPrim(exp), [evalExpCallByAllFive(A.getPrimApp2ExpArg1(exp), envir),
      ...
      while (true) {
        ...
        * /
        ...
        window.codeLines = window.codeLines || getPseudocode();

        this.expression = codeLines.join("<br />");
        ...
        try {
          expStr = undefined;
          SL.output = "";
          SL.ppm = "byval";//This determines how exp is evaluated ***** <-------
  ...
  CallByAllVal.init();
  window.CallByAllVal = CallByAllVal;
});
*/
