(function ($) {
	...
	var saveXML = function () {
    	...
    	$('#download').html('<a href="data:' + downloadData + '" target="_blank" download="fa.xml">Download Mealy</a>');
    	jsav.umsg("Saved");
	};
	    ...
	    if (xmlDoc.getElementsByTagName("type")[0].childNodes[0].nodeValue !== 'mealy') {
			...
			var nodeMap = {};			// map node IDs to nodes
	      	...
	      	for (var i = 0; i < xmlTrans.length; i++) {
	      		...
	      		if (!transout) {
	      			transout = emptystring;
	      		}
	      		else {
	      			transout = transout.nodeValue;
	      		}
	      		var winout = read + ":" + transout;
	      		var edge = g.addEdge(nodeMap[from], nodeMap[to], {weight: winout});
	      		edge.layout();
	      	}
			finalize();
	    }
