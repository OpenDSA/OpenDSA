(function ($) {
	...
	var initialize = function() {
		g = localStorage['graph']; // The graph was saved to local storage by the MooreEditor.
		if (!g) {
			jsav.umsg("Error!");
			return;
		}
		var traversal = localStorage['traversal']; // The input string was saved to local storage by the MooreEditor.
	...
	var initGraph = function(opts) {
		...
		for (var i = 0; i < gg.nodes.length; i++) {
	    	...
	    	// Add output character to the node. If it is the empty string, a function needs to be run to convert HTML5 notation to JS notation.
	    	...
	    	// If the node has a state label, it needs to be positioned correctly along with the output character.
	  	}
	  	// For each edge in the JSON, initialize it on the graph.
	  	for (var i = 0; i < gg.edges.length; i++) {
	    	if (gg.edges[i].weight !== undefined) {
	    		// Though there should be no edges with empty string transitions, this is handled here anyway.
	...
	var run = function(inputString) {
		...
		// If we made it to the end of the input string, it was accepted. Add identifying CSS to the JSAV array and graph.
		if (accepted) {
			currentState.addClass('accepted');
			arr.css(finalIndex, {"background-color": "green"});
			if (!outputString) {
				// If the output string is empty, simply note that the input string was accepted.
				jsav.umsg("<b>Accepted</b>");
			}
		}
		else {
			// If the input string was rejected, add identifying CSS to the JSAV array.
			for (var l = failingIndex; l < inputString.length - 1; l++) {
				arr.css(l, {"background-color": "red"});
			}
			arr.css(finalIndex, {"background-color": "red"});
			// Replace the output string with a note declaring that the input string was rejected.
