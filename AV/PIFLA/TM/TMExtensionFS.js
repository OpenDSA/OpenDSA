...
$(document).ready(function () {
  ...
  av.umsg("&bull; <i><b>Multiple tapes (each with its own head)</i></b> <br><br> Again, we can simulate this with encoding multiple symbols into a single table cell. For example, to simulate two tapes (each with a head), we encode in each cell the corresponding two symbols, and two binary markers to indicate if the tape head is currently in the corresponding cell of the two tapes.");

  var lab2 = av.label("&bull; <i><b>Multiple heads on one tape</i></b><br/><br/>This is easier than encoding multiple tapes. We merely encode the heads onto the tape, and simulate moving them around.", {left: 0, top: 200});
  ...
  av.umsg("&bull; <i><b>A two-dimensional \"tape\"</b></i> <br><br> All that we need to do is find a mapping from 2D to 1D, which is fairly easy. One approach is to work in diagonals, in the order (0, 0), (0, 1), (1, 0), (0, 2), (1, 1), (2, 0), and so on.");

  var lab3 = av.label("&bull; <i><b>Non-determinism</b></i> <br><br> We can simulate non-deterministic behavior in sequence, doing all length 1 computations, then length 2, etc., until we reach a halt state for one of the non-deterministic choices. So we see that while non-determinism can save a lot of time, it does not change what can (eventually) be done.", {left: 0, top: 200});
  av.recorded();
});
