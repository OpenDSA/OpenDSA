...
(function ($) {
  ...
  function prim(nodes, distances, av) {
    ...
    while (true) {
      ...
      if (nodeIndex === 0) {
        ...
        av.umsg("Select " + node.value() + ", since it has the shortest distance from A.");
      }
      av.step();
      ...
      if (prev) {
        av.umsg("Add edge (" + prev.value() + ", " + node.value() + ") to the path tree.");
      ...
      av.umsg(
        "Update " + node.value() + "'s neighbors distances. " +
        "For updated nodes, set the parent node to " + node.value()
      );
      av.step();
  ...
  function about() {
    window.alert("Dijkstra's Algorithm Proficiency Exercise\nWritten by --\nCreated as part of the OpenDSA hypertextbook project\nFor more information, see http://algoviz.org/OpenDSA\nSource and development history available at\nhttps://github.com/cashaffer/OpenDSA\nCompiled with JSAV library version " + JSAV.version());
  }
