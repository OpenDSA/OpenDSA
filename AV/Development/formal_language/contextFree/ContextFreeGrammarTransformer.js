...
transformerProto.removeUnit = function () {
  var productions = this.productions;
  var pDict = {};
...
transformerProto.removeUseless = function () {
  ...
  for (var i = 0; i < pDict[start].length; i++) {
    if (!(pDict[start][i] in visited)) {
      this.jsav.umsg("Variable(s) " (pDict[]))
  ...
  for (var i = 0; i < productions.length; i++) {
    if (_.every(productions[i][2], function(x) { return x in set || variables.indexOf(x) === -1;})) {
      if (!(productions[i][0] in set)) {
        ...
        this.jsav.umsg("The variable " + productions[i][0] + " do not produce any terminal string. It should be removed");
        this.jsav.step();
      }
    }
  }
