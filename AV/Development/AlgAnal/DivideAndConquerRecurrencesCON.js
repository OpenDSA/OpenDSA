...
$(document).ready(function () {
  ...
  var labelSet;
	...
	var valueLabel_1 = av.label("$1$", {"top": topAlign + 0.25 * nodeHeight , "left": leftAlign + nodeWidth + labelShift * 5});
	...
	var valueLabel_2 = av.label("$2$", {"top": topAlign + 1.25 * nodeHeight + nodeGap , "left": leftAlign + 3 * nodeWidth + labelShift * 5}); 
	...
	labelSet.push(av.label("$4$", {"top": topAlign + 2.25 * nodeHeight + 2 * nodeGap , "left": leftAlign + 8 * nodeWidth + labelShift * 5}));
	...
	labelSet.push(av.label("$n$", {"top": topAlign + 4.25 * nodeHeight + 4 * nodeGap , "left": leftAlign + 8 * nodeWidth + labelShift * 5}));
	...
	labelSet.push(av.label("|------------ $\\log{n + 1}$ ------------|", {"top": topAlign + 3 *nodeHeight + nodeGap, "left": leftAlign + 8.5 * nodeWidth + labelShift * 4}).addClass("rotated"));
	av.step();
	...
	//Slide 9
	av.umsg(interpret("Slide 9.1"));
	av.umsg(interpret("Slide 9.2"), {"preserve": true});
	var workLabel = av.label("<b><u>Amount of Work</u></b>", {"top": topAlign - 10 , "left": leftAlign + 250, "After": root});
	labelSet.push(workLabel);
	var dashLabel_n = av.label("----------------------------------", {"top": topAlign + 0.25 * nodeHeight, "left": leftAlign + nodeWidth + labelShift}); 
	labelSet.push(dashLabel_n);
	var valueLabel_1 = av.label("$n$", {"top": topAlign + 0.25 * nodeHeight , "left": leftAlign + nodeWidth + labelShift * 5});
	labelSet.push(valueLabel_1);
	tree = av.ds.binarytree({left: leftAlign, top: topAlign, nodegap: nodeGap});
	var root = tree.newNode("$n$");
    tree.root(root);
	...
	var dashLabel_n2 = av.label("----------------------", {"top": topAlign + 1.25 * nodeHeight + nodeGap , "left": leftAlign + nodeWidth + labelShift * 4}); 
	labelSet.push(dashLabel_n2);
	var valueLabel_2 = av.label("$n$", {"top": topAlign + 1.25 * nodeHeight + nodeGap , "left": leftAlign + 3 * nodeWidth + labelShift * 5}); 
	labelSet.push(valueLabel_2);
	var nOverFour1 = tree.newNode("$n/4$");
	var nOverFour2 = tree.newNode("$n/4$");
	var nOverFour3 = tree.newNode("$n/4$");
	...
	displaceLabels(5 * nodeWidth);
	labelSet.push(av.label("--------", {"top": topAlign + 2.25 * nodeHeight + 2 * nodeGap , "left": leftAlign + 7.5 * nodeWidth + labelShift * 4}));
	labelSet.push(av.label("$n$", {"top": topAlign + 2.25 * nodeHeight + 2 * nodeGap , "left": leftAlign + 8 * nodeWidth + labelShift * 5}));
	var nOverEight1 = tree.newNode("$n/8$");
	var nOverEight2 = tree.newNode("$n/8$");
	var nOverEight3 = tree.newNode("$n/8$");
	var nOverEight4 = tree.newNode("$n/8$");
