...
$(document).ready(function() {
  ...
  var eq = av.label("$\\frac{\\displaystyle\\sum_{j=1}^{i}j}{i}$",  {top: "-30px", left: "15px"}).addClass("mediumLabel");
  av.step();

  //Slide 6
  av.umsg(interpret("Slide 6"));
  eq.hide();
  eq = av.label("$\\begin{eqnarray*}\\displaystyle\\sum_{i=1}^{n-1}\\frac{\\displaystyle\\sum_{j=1}^{i}j}{i}\\end{eqnarray*}$", {top: "-30px", left: "15px"}).addClass("mediumLabel");
  ...
  eq.hide();
  eq = av.label(new_content,  {top: "-30px", left: "15px"}).addClass("mediumLabel");
  av.step();
