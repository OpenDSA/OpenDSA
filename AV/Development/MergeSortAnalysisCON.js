...
$(document).ready(function() {
  ...
  function merge(a1, a2, return_into) {
    ...
    for (k; k < a1.size() * 2; k++) {
      if (a1.value(i) <= a2.value(j)) {
        av.effects.moveValue(a1, s, return_into, k);
        i++;
        if (s === a1.size()) {
          for (l = j; l < a2.size(); l++) {
            k++;
  ...
  av.label("<b><u>Splitting Work</u></b>",  {top: "-22px", left: "430px"}).addClass("largeLabel");
  ...
  arr31.hide();
  arr32.hide();
  av.label("<b><u>Merging Work</u></b>",  {top: "-22px", left: "640px"}).addClass("largeLabel");
  ...
  av.label("|--------------- $\\log{n+1}$---------------|", {top: "125px", left: "250px"}).addClass("largeLabel rotated");
  av.step();
