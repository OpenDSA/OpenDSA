...
function setPointer(name, obj) {
  return obj.jsav.pointer(name, obj, {visible: true, anchor: "left top",
                                      myAnchor: "right bottom", left: 20, top: -15});
}
...
function arrowAround(node, type, options) {
  ...
  var opts = $.extend({leftOffset: nodegap / 2, rightOffset: nodegap / 2, topOffset: 15,
                       nodeGap: nodegap, nodeWidth: nodeWidth, nodeHeight: nodeHeight},
                      options);
  if (type === "top") {
    arrow = jsav.g.polyline([
      [left - opts.nodeGap - 6, top + opts.nodeHeight / 3],
    ...
    arrow = jsav.g.polyline([
      [left + opts.nodeWidth + opts.nodeGap + 6, top + opts.nodeHeight / 3 * 2],
