...
(function($) {
  ...
  var DListNode = function(container, value, options) {
    ...
    var el = $("<div><span class='jsavleftpointerarea'></span><span class='jsavvalue'>" + this._valstring(value) + "</span><span class='jsavrightpointerarea'></span></div>"),
      valtype = typeof(value);
    if (valtype === "object") { valtype = "string"; }
    this.element = el;
    el.addClass("jsavnode jsavlistnode")
        .attr({"data-value": value, "id": this._value, "data-value-type": valtype })
        .data("node", this);
  ...
  var dlistLayout = function(list, options, updateFunc) {
    ...
    if (!opts.boundsOnly) {
      ...
      for (var i = posData.length - 1; i >= 0; i--) {
		...
		if (posItem.edgePrev) {
		  posItem.edgePrev.g.movePoints(posItem.edgePos1, opts);
		  //posItem.edgePrev.g.movePoints([[1,50,28],[0,77,88]], opts);
