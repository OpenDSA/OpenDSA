...
//     }
    if (!checkTransform(strP, noLambda)) {
      strP = noLambda;
      arr = _.map(noLambda, function(x) {x = x.split(arrow); x.splice(1, 0, arrow); return x;});
      lastRow = arr.length;
      m = init();
      var noUnit = removeUnit();
      var noUseless = removeUseless();
    ...
    if (!checkTransform(strP, noUnit)) {
      strP = noUnit;
      arr = _.map(noUnit, function(x) {x = x.split(arrow); x.splice(1, 0, arrow); return x;});
      lastRow = arr.length;
      m = init();
      var noUseless = removeUseless();
      var fullChomsky = convertToChomsky();
    ...
    if (!checkTransform(strP, noUseless)) {
      strP = noUseless;
      arr = _.map(noUseless, function(x) {x = x.split(arrow); x.splice(1, 0, arrow); return x;});
      lastRow = arr.length;
      m = init();
      var fullChomsky = convertToChomsky();
    }
    if (!checkTransform(strP, fullChomsky)) {
      strP = fullChomsky;
      arr = _.map(fullChomsky, function(x) {x = x.split(arrow); x.splice(1, 0, arrow); return x;});
      lastRow = arr.length;
      m = init();
    } else {
      backup = null;
  ...
  var interactableChomsky = function (fullChomsky) {
    ...
    var chomskyHandler = function (index) {
      ...
      function checkFullChomskyConversion(curr, res) {
        if (curr.length === res.length) {
          for (var x = 0; x < curr.length; x++) {
            var line = curr[x].map(function join(a) {
              return Array.isArray(a) ? a.map(join).join('') : a;
            }).join(''); // deep joining for nested tArr
