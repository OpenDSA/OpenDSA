...
(function ($) {
  "use strict";
  var jsavArray, // JSAV array where input strings are displayed
      label = null, // Label clicked in "Edit Edges" mode.
      g, // The current JSAV graph.
      lambda = String.fromCharCode(955), // JavaScript representation of lambda.
      epsilon = String.fromCharCode(949), // JavaScript representation of epsilon.
      none = String.fromCharCode(248), // Emptyset symbol used for converting to RE
      emptystring = lambda, // Specify which empty string notation is being used.
      willRejectFunction = FiniteAutomaton.willReject, // Specify which traversal function to run (shorthand or no).
      exerciseIndex, //for creating exercises
      type = $('h1').attr('id'), //type of editor: fixer, tester or editor
      fatoreController,
      exerController, 
      jsav = new JSAV($("#jsavcontainer")); // The JSAV object

  // Handler for initializing graph upon loading the web page.
  // Loads the graph from conversionExercise.html / minimizationTest.html
  // if we are navigating here from those pages.
  ...
  function layoutTable (mat, index) {
    // if column index is given, does layout for that column,
    // otherwise lays out all columns
