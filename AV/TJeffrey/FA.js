...
var willReject = function (graph, inputString) {
  ...
  var MachineTraverseAcceptor = function(jsav, FA) {
	this.init(jsav, FA);
  };
  
  var machineAcceptorProto = MachineTraverseAcceptor.prototype;
  
  machineAcceptorProto.init = function(jsav, FA) {
	this.jsav = jsav;
	this.FA = FA;
  }	
  
  machineAcceptorProto.run = function(inputString, matrixRow) {
	// Start with the closure of the initial state.
	...
	for (var i = 0; i < inputString.length; i++) {
		...
		var yStart = 0;
		if (i == 0){
		  this.jsav.ds.tape([1, 0, 0, "", "", "", ""], 300 + 150, 20, "right");
		  var p1 = this.jsav.label("1)", {left: 270 +150, top: 0});
		  var c0 = this.jsav.g.rect(300 + 150, 100 , 110, 80);
		  var c1 = this.jsav.label("q0", {left: 320 + 150, top: 105  });
		  var c2 = this.jsav.label("q1", {left: 320 + 150, top: 135 });
		  var p3 = this.jsav.g.line(315 + 150, 100, 315 + 150, 55,
							 {"arrow-end": "classic-wide-long"});
		  var p4 = this.jsav.g.line(370 + 150, 145, 340 + 150, 135,
							 {"arrow-end": "classic-wide-long"});
		  var g = this.jsav.g.set(); 
			g.push(c0);
			g.push(p3);
			g.push(c1);
			g.push(c2);
		...
		if (i == 1){
		  this.jsav.umsg("Step 2: The tape head shifts right one cell. The current state remains in q0.");
			var p5 = this.jsav.label("2)", {left: 270 + 150, top: 0});
			p1.hide();
			p4.translateX(30);
			g.translateX(30);
			c1.translateX(30);
		...
		if (i == 2){
		  p4.hide();
			this.jsav.umsg("Step 3: The tape head shifts right one cell. The current state changes to q1.");
			var p8 = this.jsav.label("3)", {left: 270 + xStart, top: 0 + yStart});
			p5.hide();
			var p10 = this.jsav.g.line(400 + xStart, 145 + yStart, 370 + xStart, 165 + yStart,
						{"arrow-end": "classic-wide-long"});
			p10.translateX(30);
			g.translateX(30);
			c1.translateX(30);
		...
		if (i == 3){
		  this.jsav.umsg("Step 4: The tape head shifts right one cell. The current state remains in q1. Since the last character of the string has now been processed, the machine is done with this input.");
			this.jsav.label("4)", {left: 270 + xStart, top: 0 + yStart});
			p8.hide();
			p10.translateX(30);
			g.translateX(30);
			c2.translateX(30);
  ...
  machineAcceptorProto.visualize = function(listOfStrings, arrayOptions){
	...
	for(var i = 0; i< listOfStrings.length; i++){
		this.jsav.umsg("In this example, we see how the machine head moves over the tape when processing input string '100'. Step 1: Initially, the tape head is at the leftmost tape cell with character '1', and the current state is q0");
		var fake = this.jsav.label("1)", {left: 270 +150, top: 0});
		var fake2 = this.jsav.g.rect(300 + 150, 100 , 110, 80);
		var fake3 = this.jsav.label("q0", {left: 320 + 150, top: 105  });
		var fake4 = this.jsav.label("q1", {left: 320 + 150, top: 135 });
		var fake5 = this.jsav.g.line(315 + 150, 100, 315 + 150, 55,
						{"arrow-end": "classic-wide-long"});
		var fake6 = this.jsav.g.line(370 + 150, 145, 340 + 150, 135,
						{"arrow-end": "classic-wide-long"});
	  this.resetFA();
	  fake.hide();
	  fake2.hide();
  ...
  machineAcceptorProto.resetFA = function(){
  ...
  machineAcceptorProto.matrixClickHandler = function(row, column) {
  ...
  window.MachineTraverseAcceptor = MachineTraverseAcceptor;
}(jQuery));
