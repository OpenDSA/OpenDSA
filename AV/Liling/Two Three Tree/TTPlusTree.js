// Originally written by Elmer Landeverde, revised by Cliff Shaffer
...
$(document).ready(function() {
  ...
  function drawRectangle(jsav, rect, node1, node2) {
    var padding = 10;
  ...
  function drawLeafArrows(jsav, leafList, arrowList) {
  ...
  function moveNodes(nodes, left, ttop) {
    for (var i = 0; i < nodes.length; i++) {
      nodes[i].move(left, ttop);
    }
  }
   ...
   */
  function findKey(av, key, root, helper_node) {
    var curr_node = root;
    var prev_node = null;
  ...
  var TTTree = {};
  TTTree.drawRectangle = drawRectangle;
  TTTree.drawLeafArrows = drawLeafArrows;
  TTTree.findKey = findKey;
  TTTree.moveNodes = moveNodes;
  window.TTTree = TTTree;
});
...
$(document).ready(function() {
  ...
  function newNode(jsav, keys, isLeaf, values) {
    if (isLeaf) {
    ...
    return new TTNode(jsav, arr, isLeaf).center();
  }

  // Node constructor.
  function TTNode(jsav, arr, isLeaf) {
    this.jsav = jsav;
    this.array = arr;
    if (isLeaf) {
      this.isLeaf = isLeaf;
  ...
  var TTNodeproto = TTNode.prototype;

  // Get/Set node value.
  TTNodeproto.value = function(idx, key, value) {
    if (!key && key !== "") {
  ...
  TTNodeproto.addChild = function(newChild) {
  ...
  TTNodeproto.insertChild = function(idx, newChild) {
    this.children.splice(idx, 0, newChild);
    this.edges.splice(idx, 0, null);
  };

  // Remove child at specified index.
  TTNodeproto.removeChild = function(idx) {
  ...
  TTNodeproto.drawEdge = function(child_idx) {
  ...
  TTNodeproto.updateEdges = function(recursive) {
  ...
  // Hide all edges.
  TTNodeproto.hideEdges = function() {
    for (var i = 0; i < this.edges.length; i++) {
      this.edges[i].hide();
    }
    return this;
  ...
  TTNodeproto.hideEdge = function(idx) {
    this.edges[idx].hide();
  };

  // Show all edges.
  TTNodeproto.showEdges = function() {
    for (var i = 0; i < this.edges.length; i++) {
      this.edges[i].show();
    }
    return this;
  ...
  TTNodeproto.showEdge = function(idx) {
    this.edges[idx].show();
    return this;
  };

  // Toggle node highlighting.
  TTNodeproto.highlightToggle = function() {
  ...
  TTNodeproto.highlightToggleEdge = function(idx) {
    this.edges[idx].toggleClass("highlight-edge");
  };

  // Un-highlight all edges of this node.
  TTNodeproto.unhighlightEdges = function() {
    for (var i = 0; i < this.edges.length; i++) {
  ...
  TTNodeproto.child = function(pos) {
    return this.children[pos];
  };

  // Publicize the public functions
  var TTTreeNode = {};
  TTTreeNode.newNode = newNode;
  window.TTTreeNode = TTTreeNode;
});
