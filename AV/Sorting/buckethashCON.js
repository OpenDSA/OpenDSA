...
(function ($) {
  ...
  setGray(arr, 8);
  av.step();
  av.umsg("Since there are 10 slots, we take 30 % 10 to get 0. Put this into slot 0, which is in bucket 0.");
  arr.highlight(0);
  arr.value(0, 30);
  av.step();
  ...
  setGray(arr, 0);
  av.step();
  av.umsg("Since there are 10 slots, we take 21 % 10 to get 1. Put this into slot 1, which is in bucket 0.");
  arr.highlight(1);
  arr.value(1, 21);
  av.step();
  av.umsg("Now insert a record with key value 48.");
