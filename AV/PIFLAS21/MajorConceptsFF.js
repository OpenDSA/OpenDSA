$(document).ready(function() {
  ...
  av.step();
  //Frame 18
  av.umsg("Sometimes we want to talk about the string that has no characters. If we literally wrote a string with no characters, it would be hard for you to understand what we wrote! So we have a special symbol to use for the empty string: $\\lambda$ and it is called <b>lambda</b>.");
  av.step();
  //Frame 19
  av.umsg(Frames.addQuestion("q8_1"));
  av.step();
  // Frame 19.1
  av.umsg("Note the difference. $L_1 = \\{\\}$ is the language with no strings in it. But $L_2 = \\{ \\lambda \\}$ is the language that has just the empty string. The empty string is a perfectly fine string, and a language with the empty string is different from a language with no strings at all.");
  av.step();
  //Frame 20
