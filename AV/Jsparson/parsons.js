(function ($, _) { // wrap in anonymous function to not show some helper variables
    ...
    ParsonsWidget.prototype.getHashArr = function (searchString) {
        var hash = [],
            ids = $(searchString).sortable('toArray'),
            line;
        for (var i = 0; i < ids.length; i++) {
    ...
    ParsonsWidget.prototype.solutionHashArr = function () {
        return this.getHashArr("#ul-" + this.options.sortableId);
    };
    ...
    ParsonsWidget.prototype.getState = function (entry) {
        const orderObj = {[entry.index]: this.solutionHashArr()}
        const orderJson = JSON.stringify(orderObj, null, 2)
        return orderJson
    };
