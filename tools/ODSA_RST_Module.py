...
def format_mod_options(options):
    ...
    for option, value in options.items():
        ...
        if option.startswith('JXOP-'):
            option_str += "JSAV_EXERCISE_OPTIONS['%s']=%s;" % (
                option[5:], value)
...
def parse_directive_args(line, line_num, expected_num_args=-1, console_msg_prefix=''):
    ...
    if len(directive) != 2 and directive[0] != '..':
        print_err("%sERROR: Invalid Sphinx directive declaration" %
                  console_msg_prefix)
    ...
    if expected_num_args > -1 and len(args) != expected_num_args:
        ...
        print_err("%sERROR: Invalid directive arguments for object on line %d, skipping object" % (
            console_msg_prefix, line_num))
...
def parse_term_relationship(line, term, line_num, cmap_dict, console_msg_prefix=''):
    if line.strip().startswith(':to-term:') and ':label:' in line.lower():
        ...
        print_err("%sWARNING: Glossary terms relationship declaration on line %d" % (
            console_msg_prefix, line_num))
...
class ODSA_RST_Module:
    ...
    def __init__(self, config, mod_path, mod_attrib={'exercises': {}}, satisfied_requirements=[], chap='', depth=0, current_section_numbers=[], standalone_modules=False):
        ...
        dispModComp = determine_module_completable(
            mod_attrib, config.dispModComp)

        filename = '{0}RST/{1}/{2}.rst'.format(
            config.odsa_dir, config.lang, mod_path)
        ...
        if not os.path.exists(filename):
            ...
            while i < len(mod_data):
                ...
                if next_line.startswith("=") or next_line.startswith("-") or next_line.startswith("~"):
                    ...
                    is_chapter = next_line == "=" * \
                        len(next_line) and len(next_line) > 0
                    is_section = next_line == "-" * \
                        len(next_line) and len(next_line) > 0
                    is_sub_section = next_line == "~" * \
                        len(next_line) and len(next_line) > 0
                else:
                    next_line = next_line.strip()
                    is_chapter = False
                ...
                if is_chapter or is_section or is_sub_section:
                    ...
                    if not content_before_module \
                        ...
                        errors.append(("%sERROR: %s: line %s ('%s') - should not have content before module title" % (
                            console_msg_prefix, mod_path, i, line), True))
                ...
                if is_section \
                    ...
                    print('%sRemoving section: %s' %
                          (console_msg_prefix, line))
                    del mod_data[i]
                    del mod_data[i]
                    line = mod_data[i].strip()
                    next_line = mod_data[i+1].strip() if i + \
                        1 < len(mod_data) else ''
                ...
                if is_chapter:
                    ...
                    if is_section:
                        ...
                        errors.append(("%sERROR: %s: line %s ('%s') - exercises must be inside a section" % (
                            console_msg_prefix, mod_path, i, line), True))
                ...
                if dir_type in ['table', 'example', 'theorem', 'figure']:
                    (num_ref_map, counters) = update_counters(
                        mod_data[i - 2], dir_type, mod_num, num_ref_map, counters)
                ...
                if ':ref:' in line:
                    ...
                    mod_data[i] = process_ref_chap(
                        ':ref:', line, config.listed_modules, start_space, last, standalone_modules)
                ...
                if ':chap:' in line:
                    if mod_data[i].endswith('\n'):
                        ...
                        last = ' '
                    mod_data[i] = process_ref_chap(
                        ':chap:', line, config.listed_chapters, start_space, last, standalone_modules)
                    line = mod_data[i].strip().lower()

                if ':requires:' in mod_data[i]:
                    # Parse the list of prerequisite topics from the module
                    requires = [req.strip() for req in line.replace(
                        ':requires:', '').split(';')]

                    # Print a warning message if a missing prereq is encountered
                    for req in requires:
                        if req != '' and req not in satisfied_requirements:
                            print_err("%sWARNING: %s is an unsatisfied prerequisite for %s, line %d" % (
                                console_msg_prefix, req, mod_name, i + 1))
                    ...
                    while (i < len(mod_data) and (mod_data[i].strip() != '' or mod_data[i] not in ['\n', '\r\n'])):
                        if mod_data[i].strip().startswith(':to-term:'):
                            parse_term_relationship(
                                term_rel, term, line_num, cmap_dict, console_msg_prefix='')
                            term_rel = mod_data[i]
                            line_num = i
                        else:
                            term_rel += mod_data[i]
                    ...
                    if config.suppress_todo:
                        ...
                        todo_list.append(
                            (todo_id, mod_name, todo_type, todo_directive))
                    ...
                    args = parse_directive_args(
                        mod_data[i], i, 2, console_msg_prefix)
                    ...
                    if args:
                        ...
                        if av_type in ['ss', 'ff', 'dgm']:
                            ...
                            while opt_line.startswith(':'):
                                ...
                                ) if j < len(mod_data) else ''

                            if not scripts_found and os.environ['SLIDES'] == 'no':
                                print_err("{0}WARNING: Module '{1}' -- inlineav '{2}' missing :scripts: option".format(
                                    console_msg_prefix, mod_path, av_name))
                        ...
                        if av_type == 'ss':
                            if av_name not in exercises:
                                ...
                                options = ['points', 'required',
                                           'threshold', 'id']

                                rst_options = [' '*start_space + '   :%s: %s\n' % (option, str(
                                    exer_conf[option])) for option in options if option in exer_conf]
                        ...
                        if av_type == 'ff':
                            ...
                            print_err("%sWARNING: %s is a diagram (attributes are not supported), line %d" % (
                                console_msg_prefix, av_name, i + 1))
                        elif av_type not in ['ff', 'ss', 'dgm']:
                            # If a warning if the exercise type doesn't match something we expect
                            print_err("%sWARNING: Unsupported type '%s' specified for %s, line %d" % (
                                console_msg_prefix, av_type, av_name, i + 1))
                    ...
                    if args:
                        ...
                        if not os.path.exists('{0}/{1}'.format(config.odsa_dir, av_path)):
                            print_err("{0}WARNING: In module '{1}' -- the avembed file '{2}' does not exist.".format(
                                console_msg_prefix, mod_path, av_path))

                        av_name = os.path.splitext(
                            os.path.basename(av_path))[0]
                        ...
                        if av_name in exercises and 'remove' in exercises[av_name] and exercises[av_name]['remove']:
                            ...
                            if av_name not in exercises:
                                ...
                                options = ['points', 'required', 'showhide',
                                           'threshold', 'external_url', 'id']
                                ...
                                if 'exer_options' in exer_conf:
                                    xops = dict(list(config.glob_exer_options.items(
                                    )) + list(exer_conf['exer_options'].items()))
                                else:
                                ...
                                xop_str = '&amp;'.join(['%s=%s' % (option, value) if str(value) not in [
                                                       'True', 'False'] else '%s=%s' % (option, str(value).lower()) for option, value in xops.items()])
                                rst_options.append(
                                    ' '*start_space + '   :exer_opts: %s\n' % xop_str)
                    ...
                    if args:
                        ...
                        if external_tool_name not in exercises:
                            ...
                            options = [
                                option for option in options if option not in dir_opts]
                    ...
                    if 'sections' in mod_attrib and section_id in mod_attrib['sections']:
                        ...
                        if 'remove' in section_data and section_data['remove']:
                            ...
                            ignore_opts = ['remove']
                            rst_options = ['   :%s: %s\n' % (option, value) for option, value in list(
                                section_data.items()) if option not in ignore_opts]
                            mod_data[i] += ''.join(rst_options)
                    ...
                    i += 1
                    equation = re.split(':label:', mod_data[i], re.IGNORECASE)[
                        1].strip()
                    pattern = '%s#'
                    if mod_num:
                        pattern = '.' + pattern
                    num_ref_map['equation-' + equation] = mod_num + \
                        pattern % str(counters['equation'])
                    counters['equation'] += 1
                elif ':target:' in line:
            ...
            if not avmetadata_found:
                print_err("%sWARNING: %s does not contain an ..avmetadata:: directive" % (
                    console_msg_prefix, mod_name))
            ...
            )) if 'sections' in mod_attrib and mod_attrib['sections'] != None else []

            # Print a list of sections that appear in the config file but not the module
            missing_sections = list(
                set(mod_sections) - set(processed_sections))

            for section in missing_sections:
                print_err('%sWARNING: Section "%s" not found in module "%s"' % (
                    console_msg_prefix, section, mod_path))
